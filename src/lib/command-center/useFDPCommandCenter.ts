/**
 * FDP COMMAND CENTER HOOK
 * 
 * Independent Command Center for Finance decisions.
 * Fetches only FDP-domain decisions from decision_cards table.
 * 
...
import { buildFDPDecision } from './decision-builders';

// ═══════════════════════════════════════════════════════════════════
// TYPES
// ═══════════════════════════════════════════════════════════════════

export interface FDPCommandCenterFilters {
...
export interface FDPCommandCenterResult {
...
const FDP_CARD_TYPES = [
  'FDP_CASH_BURN',
  'FDP_MARGIN_DECLINE',
  'FDP_REVENUE_DROP',
  'FDP_EXPENSE_SPIKE',
  'FDP_DSO_INCREASE',
  'FDP_CCC_DETERIORATION',
  'FDP_EBITDA_MISS',
  'FDP_WORKING_CAPITAL',
  'FDP_GROSS_MARGIN',
  'FDP_NET_MARGIN',
...
export function useFDPCommandCenter(
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['fdp-command-center', tenantId, status, severity, ownerRole, limit],
...
export function useFDPDecide() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Đã ghi nhận quyết định FDP');
    },
    onError: (error) => {
      console.error('FDP Decision error:', error);
...
export function useFDPEscalate() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      console.error('FDP Escalation error:', error);
...
function transformToDecisionContract(
  ...
  return {
    ...
    domain,
    decision_type: (r.card_type as string) || 'FDP_UNKNOWN',
    ...
    owner_role: ((r.owner_role as string) || 'CFO') as DecisionOwnerRole,
