...
export function useDecideCard() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // Also log to unified audit log for DB cards
      await supabase.from('decision_audit_log').insert({
        tenant_id: tenantId!,
        card_id: cardId,
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data: currentCard } = await supabase
        .from('decision_cards')
        .select('snooze_count, card_type, entity_type, entity_id, entity_label, impact_amount, impact_currency')
        .eq('id', cardId)
        .single();
      ...
      await supabase.from('decision_audit_log').insert({
        tenant_id: tenantId!,
        card_id: cardId,
        card_type: currentCard?.card_type || 'UNKNOWN',
        entity_type: currentCard?.entity_type || 'UNKNOWN',
        entity_id: currentCard?.entity_id,
        entity_label: currentCard?.entity_label,
        action_type: 'SNOOZE',
        decision_status: 'SNOOZED',
        decided_by: user?.id,
        decided_at: new Date().toISOString(),
        snoozed_until: snoozedUntil.toISOString(),
        impact_amount: currentCard?.impact_amount,
        impact_currency: currentCard?.impact_currency || 'VND',
      });
