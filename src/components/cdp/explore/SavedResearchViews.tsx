...
import { useCDPSavedViews } from '@/hooks/useCDPExplore';
...
export function SavedResearchViews() {
  const { data: savedViews, isLoading } = useCDPSavedViews();
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {views.length} góc nhìn • Được sử dụng cho phân tích và quyết định
                </p>
              </div>
            </div>
          </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge 
                        variant="outline" 
                        className={cn("text-xs", purposeLabels[view.purpose]?.className || purposeLabels.exploration.className)}
                      >
                        {purposeLabels[view.purpose]?.label || 'Khám phá'}
                      </Badge>
                    </div>
                    {view.description && (
                    ...
                    <div className="flex items-center gap-6 text-xs text-muted-foreground">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>{view.creator || 'Unknown'}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
