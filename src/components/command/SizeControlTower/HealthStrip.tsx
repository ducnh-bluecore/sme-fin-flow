...
export default function HealthStrip({
  ...
  const damageMetrics = [
    { icon: ShieldAlert, label: 'Lẻ Size', value: brokenCount, sub: riskCount > 0 ? `+${riskCount} rủi ro` : undefined, color: 'text-destructive', borderColor: 'border-l-destructive', tooltip: 'Số mẫu có đường cong size bị gãy, không bán được đầy đủ kích cỡ' },
    { icon: DollarSign, label: 'DT Mất', value: totalLostRevenue > 0 ? formatVNDCompact(totalLostRevenue) : '—', color: 'text-destructive', borderColor: 'border-l-destructive', tooltip: 'Doanh thu ước tính bị mất do thiếu size (không bán được cho khách muốn mua)' },
    { icon: Lock, label: 'Vốn Khóa', value: totalCashLocked > 0 ? formatVNDCompact(totalCashLocked) : '—', color: 'text-orange-500', borderColor: 'border-l-orange-500', tooltip: 'Giá vốn hàng tồn bị kẹt trong các mẫu lẻ size — tiền thực đang bị khóa, không sinh lời' },
    { icon: Flame, label: 'Rò Biên', value: totalMarginLeak > 0 ? formatVNDCompact(totalMarginLeak) : '—', color: 'text-red-500', borderColor: 'border-l-red-500', tooltip: 'Dự phóng mất biên lợi nhuận nếu phải markdown (tính theo xác suất 50% × giá bán lẻ × mức giảm 35%)' },
  ];
    ...
    <TooltipProvider delayDuration={200}>
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-center gap-6">
          ...
          {/* Center: Damage Metrics with tooltips */}
              ...
              <Tooltip key={m.label}>
                <TooltipTrigger asChild>
                  <motion.div
                    ...
                    className={`rounded-lg bg-card/50 border border-border/50 border-l-[3px] ${m.borderColor} p-3 transition-all duration-300 hover:bg-card/80 cursor-help`}
                  >
