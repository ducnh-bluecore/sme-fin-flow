/**
 * BudgetVsActualContent - Extracted from BudgetVsActualPage
 * This component displays budget vs actual comparison
 * Used in PerformanceAnalysisPage as a tab
 */

import { useState } from 'react';
import { 
  Info, ChevronDown, FileSpreadsheet, AlertTriangle, ExternalLink
} from 'lucide-react';
          ...
          <Card className="bg-muted/30">
            <CardContent className="pt-4 grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <p className="text-muted-foreground text-xs mt-1">{t('budgetVsActual.plannedSource')}</p>
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs mt-1">{t('budgetVsActual.actualRevenueSource')}</p>
                <p className="text-xs text-muted-foreground">{t('budgetVsActual.actualExpenseSource')}</p>
              </div>
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">{t('budgetVsActual.positiveGood')}</p>
              </div>
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">{t('budgetVsActual.savingsGood')}</p>
              </div>
            </CardContent>
          </Card>
        </CollapsibleContent>
        ...
        {!hasPlannedData ? (
          <Alert>
            ...
            <AlertDescription className="flex items-center justify-between gap-3">
              <span>{t('budgetVsActual.noPlanAIWarning')}</span>
              <Button asChild variant="outline" size="sm">
          ...
          <ContextualAIPanel context="budget_vs_actual" title={t('budgetVsActual.aiTitle')} />
        )}
      </section>
    </div>
  );
