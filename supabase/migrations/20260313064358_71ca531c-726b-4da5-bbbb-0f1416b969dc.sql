...
CREATE OR REPLACE FUNCTION public.get_channel_pl_computed(p_tenant_id uuid, p_start_date date, p_end_date date)
 RETURNS TABLE(channel text, total_revenue numeric, total_fees numeric, total_cogs numeric, gross_profit numeric, operating_profit numeric, order_count bigint, avg_order_value numeric, gross_margin numeric, operating_margin numeric, revenue_share numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
