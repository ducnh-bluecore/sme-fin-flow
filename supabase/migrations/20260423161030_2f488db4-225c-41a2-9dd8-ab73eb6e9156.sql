...
-- ============================================================
-- 1. Seed constraint data
-- ============================================================

-- V1 allocation matrix (tier × CW stock)
INSERT INTO inv_constraint_registry (tenant_id, constraint_key, constraint_value, description, is_active, version)
SELECT t.id, 'v1_allocation_matrix',
  '{"ranges":[{"max_cw":29,"S":0,"A":0,"B":0,"C":0},{"max_cw":40,"S":3,"A":2,"B":0,"C":0},{"max_cw":60,"S":4,"A":2,"B":2,"C":1},{"max_cw":80,"S":4,"A":2,"B":2,"C":1},{"max_cw":100,"S":5,"A":3,"B":2,"C":2},{"max_cw":999999,"S":6,"A":4,"B":3,"C":2}]}'::jsonb,
  'Ma trận phân bổ V1 theo Tier × Tồn CW. Mỗi dòng: max_cw là ngưỡng tồn CW, S/A/B/C là số lượng min_stock tương ứng.',
  true, 1
FROM tenants t
WHERE NOT EXISTS (SELECT 1 FROM inv_constraint_registry cr WHERE cr.tenant_id = t.id AND cr.constraint_key = 'v1_allocation_matrix');

-- CW min threshold
INSERT INTO inv_constraint_registry (tenant_id, constraint_key, constraint_value, description, is_active, version)
SELECT t.id, 'cw_min_threshold',
  '{"min_qty": 6}'::jsonb,
  'Ngưỡng tồn CW tối thiểu. FC nào có tồn CW < min_qty sẽ không được phân bổ.',
  true, 1
FROM tenants t
WHERE NOT EXISTS (SELECT 1 FROM inv_constraint_registry cr WHERE cr.tenant_id = t.id AND cr.constraint_key = 'cw_min_threshold');

-- V2 order cap by tier
INSERT INTO inv_constraint_registry (tenant_id, constraint_key, constraint_value, description, is_active, version)
SELECT t.id, 'v2_order_cap_by_tier',
  '{"S": 6, "A": 4, "B": 3, "C": 2}'::jsonb,
  'Giới hạn số lượng tối đa V2 theo tier cửa hàng.',
  true, 1
FROM tenants t
WHERE NOT EXISTS (SELECT 1 FROM inv_constraint_registry cr WHERE cr.tenant_id = t.id AND cr.constraint_key = 'v2_order_cap_by_tier');

-- Update SEM allocation policies weights
UPDATE sem_allocation_policies SET weights = '{"tier_S": 1.5, "tier_A": 1.2, "tier_B": 1.0, "tier_C": 0.7}'::jsonb
WHERE policy_type = 'BASE' AND is_active = true;

UPDATE sem_allocation_policies SET weights = '{"tier_S": 1.5, "tier_A": 1.2, "tier_B": 1.0, "tier_C": 0.7}'::jsonb
WHERE policy_type = 'DYNAMIC' AND is_active = true;

-- ============================================================
-- 2. Recreate fn_allocation_engine with OLV-only + matrix + caps
-- ============================================================
...
BEGIN
  ...
  -- NEW: V2 tier cap
  SELECT COALESCE((SELECT constraint_value FROM inv_constraint_registry WHERE tenant_id=p_tenant_id AND constraint_key='v2_order_cap_by_tier' AND is_active=true LIMIT 1), '{"S":6,"A":4,"B":3,"C":2}'::jsonb) INTO v_v2_tier_cap;
  ...
  -- CHANGED: Added tier filter - only OLV stores (S/A/B/C)
  ...
  IF p_run_type IN ('V1','both') THEN
    WITH v1_raw AS (
      SELECT f.id AS fid, f.fc_name, f.fc_code, f.is_core_hero, f.criticality_class, f.sku_count,
        ...
        -- NEW: Lookup min_stock from allocation matrix
    ...
    v1_ranked AS (
      SELECT *,
        ...
        SUM(matrix_min_stock) OVER (PARTITION BY fid ORDER BY tord, vel DESC, cs ASC) AS cum_qty
      FROM v1_raw
    ...
    SELECT p_tenant_id,p_run_id,fid,fc_name||' ('||fc_code||')',sid,store_name||' ('||store_code||')',
      ...
      'pending',
      GREATEST(1, matrix_min_stock) * v_avg_unit_price,
      jsonb_build_object('source_on_hand',cwt,'dest_on_hand',cs,'sold_7d',ROUND(vel*7,1),'tier_weight',ROUND(tier_weight,2),'criticality',criticality_class,'matrix_min_stock',matrix_min_stock),
      'V1'
    FROM v1_ok
  ...
  IF p_run_type IN ('V2','both') THEN
    ...
    v2_ranked AS (
      SELECT *,
        -- CHANGED: Apply tier cap to need
        LEAST(
          tier_cap,
    ...
    SELECT p_tenant_id,p_run_id,fid,fc_name||' ('||fc_code||')',sid,store_name||' ('||store_code||')',
      ...
      CASE WHEN is_bst_new AND vel = 0 THEN 'V2-BST mới (phủ nền, chưa có sales) ×'||ROUND(dyn_weight,2)||' cap='||tier_cap
           WHEN co>0 THEN 'V2-Đơn KH: '||co||' pcs, vel='||ROUND(vel,2)||'/ngày (×'||ROUND(dyn_weight,2)||') cap='||tier_cap
           ELSE 'V2-Nhu cầu: vel='||ROUND(vel,2)||'/ngày, cover='||ROUND(wc,1)||'d (×'||ROUND(dyn_weight,2)||') cap='||tier_cap END,
      ...
      jsonb_build_object('source_on_hand',cwt,'dest_on_hand',cs,'sold_7d',ROUND(vel*7,1),'customer_orders',co,'store_orders',so,'dyn_weight',ROUND(dyn_weight,2),'criticality',criticality_class,'is_bst_new',is_bst_new,'tier_cap',tier_cap),
      'V2'
    FROM v2_ok
    ON CONFLICT (run_id, fc_id, store_id) DO NOTHING;
...
-- ============================================================
-- 3. Recreate fn_rebalance_engine with OLV-only filter
-- ============================================================
CREATE OR REPLACE FUNCTION public.fn_rebalance_engine(
  p_tenant_id uuid,
  p_run_id uuid
) RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  -- CHANGED: Added tier filter to _pos for destination stores (S/A/B/C only)
  ...
  JOIN inv_stores s ON s.id = p.store_id AND s.is_active
    AND (s.location_type = 'central_warehouse' OR s.tier IN ('S','A','B','C'))
