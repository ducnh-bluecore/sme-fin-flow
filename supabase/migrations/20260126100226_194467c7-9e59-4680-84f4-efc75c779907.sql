
-- Fix refresh_dashboard_kpi_cache with correct column names
-- SSOT: total_revenue = net_revenue, use correct invoice columns

CREATE OR REPLACE FUNCTION public.refresh_dashboard_kpi_cache(
  p_tenant_id uuid,
  p_date_range integer DEFAULT 90
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_start_date date;
  v_end_date date;
  v_cash_today numeric;
  v_cash_7d numeric;
  v_total_ar numeric;
  v_overdue_ar numeric;
  v_dso integer;
  v_ccc integer;
  v_gross_margin numeric;
  v_ebitda numeric;
  v_auto_match_rate numeric;
  v_net_revenue numeric;
  v_total_cogs numeric;
  v_total_opex numeric;
  v_invoice_count integer;
  v_overdue_invoice_count integer;
  v_transaction_count integer;
  v_matched_transaction_count integer;
BEGIN
  -- SSOT: Uses cdp_orders as single source of truth for order data
  
  v_end_date := CURRENT_DATE;
  v_start_date := v_end_date - p_date_range;

  -- Revenue, COGS, Margin from cdp_orders (SSOT)
  SELECT 
    COALESCE(SUM(net_revenue), 0),
    COALESCE(SUM(cogs), 0),
    COALESCE(SUM(gross_margin), 0)
  INTO v_net_revenue, v_total_cogs, v_gross_margin
  FROM cdp_orders
  WHERE tenant_id = p_tenant_id
    AND order_at >= v_start_date
    AND order_at <= v_end_date;

  -- OPEX from expenses
  SELECT COALESCE(SUM(amount), 0)
  INTO v_total_opex
  FROM expenses
  WHERE tenant_id = p_tenant_id
    AND expense_date >= v_start_date
    AND expense_date <= v_end_date;

  -- Cash from bank_accounts
  SELECT COALESCE(SUM(current_balance), 0)
  INTO v_cash_today
  FROM bank_accounts
  WHERE tenant_id = p_tenant_id
    AND status = 'active';

  -- Cash flow 7d from bank_transactions
  SELECT COALESCE(SUM(CASE 
    WHEN transaction_type = 'credit' THEN amount 
    ELSE -amount 
  END), 0)
  INTO v_cash_7d
  FROM bank_transactions
  WHERE tenant_id = p_tenant_id
    AND transaction_date >= (v_end_date - 7)
    AND transaction_date <= v_end_date;

  -- AR metrics: total_amount - paid_amount = amount receivable
  SELECT 
    COALESCE(SUM(total_amount - COALESCE(paid_amount, 0)), 0),
    COALESCE(SUM(CASE WHEN due_date < CURRENT_DATE THEN (total_amount - COALESCE(paid_amount, 0)) ELSE 0 END), 0),
    COUNT(*),
    COUNT(CASE WHEN due_date < CURRENT_DATE THEN 1 END)
  INTO v_total_ar, v_overdue_ar, v_invoice_count, v_overdue_invoice_count
  FROM invoices
  WHERE tenant_id = p_tenant_id
    AND status != 'paid';

  -- Transaction matching from bank_transactions
  SELECT 
    COUNT(*),
    COUNT(CASE WHEN match_status = 'matched' THEN 1 END)
  INTO v_transaction_count, v_matched_transaction_count
  FROM bank_transactions
  WHERE tenant_id = p_tenant_id
    AND transaction_date >= v_start_date;

  v_auto_match_rate := CASE 
    WHEN v_transaction_count > 0 
    THEN (v_matched_transaction_count::numeric / v_transaction_count) * 100 
    ELSE 0 
  END;

  v_dso := 0;
  v_ccc := 0;
  v_ebitda := v_net_revenue - v_total_cogs - v_total_opex;

  -- Upsert cache: total_revenue = net_revenue (SSOT)
  INSERT INTO dashboard_kpi_cache (
    tenant_id, cash_today, cash_7d, total_ar, overdue_ar,
    dso, ccc, gross_margin, ebitda, auto_match_rate,
    net_revenue, total_revenue, total_cogs, total_opex,
    invoice_count, overdue_invoice_count, transaction_count, matched_transaction_count,
    date_range_start, date_range_end, calculated_at, updated_at
  ) VALUES (
    p_tenant_id, v_cash_today, COALESCE(v_cash_7d, v_cash_today), v_total_ar, v_overdue_ar,
    v_dso, v_ccc, v_gross_margin, v_ebitda, v_auto_match_rate,
    v_net_revenue, v_net_revenue, v_total_cogs, v_total_opex,
    v_invoice_count, v_overdue_invoice_count, v_transaction_count, v_matched_transaction_count,
    v_start_date, v_end_date, now(), now()
  )
  ON CONFLICT (tenant_id) DO UPDATE SET
    cash_today = EXCLUDED.cash_today,
    cash_7d = EXCLUDED.cash_7d,
    total_ar = EXCLUDED.total_ar,
    overdue_ar = EXCLUDED.overdue_ar,
    gross_margin = EXCLUDED.gross_margin,
    ebitda = EXCLUDED.ebitda,
    auto_match_rate = EXCLUDED.auto_match_rate,
    net_revenue = EXCLUDED.net_revenue,
    total_revenue = EXCLUDED.total_revenue,
    total_cogs = EXCLUDED.total_cogs,
    total_opex = EXCLUDED.total_opex,
    invoice_count = EXCLUDED.invoice_count,
    overdue_invoice_count = EXCLUDED.overdue_invoice_count,
    transaction_count = EXCLUDED.transaction_count,
    matched_transaction_count = EXCLUDED.matched_transaction_count,
    calculated_at = now(),
    updated_at = now();
END;
$$;

-- Refresh E2E tenant cache
SELECT refresh_dashboard_kpi_cache('aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee'::uuid, 90);
