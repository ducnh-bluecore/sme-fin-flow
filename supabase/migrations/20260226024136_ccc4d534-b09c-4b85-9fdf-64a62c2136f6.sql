...
-- Fix: cast missing_sizes text[] to jsonb in the SCS insert
...
BEGIN
  ...
  SELECT p.store_id, p.fc_id, p.sku, p.on_hand, p.weeks_of_cover
  ...
  SELECT d.store_id, d.fc_id, d.avg_daily_sales
  ...
  SELECT m.fc_id, m.sku, m.size AS size_code FROM inv_sku_fc_mapping m WHERE m.tenant_id = p_tenant_id AND m.is_active = TRUE;
  ...
  WITH store_doc AS (
    ...
    SELECT sd.fc_id, SUM(CASE WHEN sd.doc>fs.mean_doc*1.5 THEN sd.on_hand*150000 ELSE 0 END) AS locked_cash,
      to_jsonb(ARRAY_AGG(DISTINCT sd.store_id) FILTER (WHERE sd.doc>fs.mean_doc*1.5)) AS ol,
      to_jsonb(ARRAY_AGG(DISTINCT sd.store_id) FILTER (WHERE sd.doc<fs.mean_doc*0.5 AND sd.doc<14)) AS ul
  ...
  WITH fss AS (
    SELECT m.fc_id,m.size_code,SUM(p.on_hand) AS tq FROM _positions p JOIN _sku_map m ON m.sku=p.sku JOIN _multi_size_fcs msf ON msf.fc_id=m.fc_id WHERE m.size_code IS NOT NULL GROUP BY m.fc_id,m.size_code
  ...
  SELECT p_tenant_id,fc_id,NULL,p_as_of_date,
    GREATEST(0,LEAST(100,100-LEAST(40,ds*20)-CASE WHEN cm THEN 30 ELSE 0 END-sc*5)) AS score,
  ...
  SELECT p_tenant_id,sh.product_id,p_as_of_date,'size_intelligence',
    ...
    CONCAT_WS(' | ','Health: '||ROUND(sh.size_health_score)||' ('||sh.curve_state||')',
      ...
      CASE WHEN md.markdown_risk_score IS NOT NULL THEN 'MD Risk: '||md.markdown_risk_score END),
