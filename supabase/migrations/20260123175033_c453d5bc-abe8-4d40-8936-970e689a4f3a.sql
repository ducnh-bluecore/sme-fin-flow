...
-- Fix function with correct column names (equity_12m instead of ltv_12m)
...
BEGIN
  ...
  LOOP
    ...
    IF v_segment.name = 'VIP Customers' THEN
      ...
      INSERT INTO cdp_segment_membership_daily (tenant_id, as_of_date, segment_id, customer_id, is_member)
      SELECT p_tenant_id, p_as_of_date, v_segment.id, eq.customer_id, true
    ...
    ELSIF v_segment.name = 'Discount Hunters' THEN
      INSERT INTO cdp_segment_membership_daily (tenant_id, as_of_date, segment_id, customer_id, is_member)
      SELECT p_tenant_id, p_as_of_date, v_segment.id, r.customer_id, true
    ...
    ELSIF v_segment.name = 'Shopee Loyalists' THEN
      -- Skip for now - need channel data
      v_inserted := 0;
