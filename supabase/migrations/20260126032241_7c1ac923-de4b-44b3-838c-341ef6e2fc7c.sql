...
-- Fix control_tower_aggregate_signals - use correct column names
...
BEGIN
  ...
  -- 2. Aggregate early warning alerts (use exposure_amount not impact_amount)
  FOR v_record IN
    SELECT 
      e.id,
      e.metric_code,
      CASE 
        ...
        ELSE 30
      END as priority_score,
      COALESCE(e.exposure_amount, 0) as impact,
      e.title,
