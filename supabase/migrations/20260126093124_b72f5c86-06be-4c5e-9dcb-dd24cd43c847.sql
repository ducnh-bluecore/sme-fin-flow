...
-- Fix v_fdp_finance_summary: Remove 90-day restriction to handle all historical data
-- The view was returning 0 because all orders were outside the 90-day window
...
WITH order_metrics AS (
  ...
  -- No date restriction - include all historical data for finance summary
  GROUP BY tenant_id
),
expense_metrics AS (
  SELECT 
...
SELECT 
  o.tenant_id,
  -- Keep original column order to match types.ts
  ...
  -- CAC (placeholder - marketing spend / unique customers)
