...
-- Update V2 tier caps: reduce C and B
UPDATE inv_constraint_registry 
SET constraint_value = '{"S": 6, "A": 4, "B": 2, "C": 1}'::jsonb,
    updated_at = now()
WHERE constraint_key = 'v2_order_cap_by_tier';

-- Recreate fn_allocation_engine with tier-priority V2 ordering
...
BEGIN
  ...
  IF p_run_type IN ('V2','both') THEN
    ...
    v2_ranked AS (
      SELECT *,
        -- CHANGED: Tier-first ordering - S gets allocated before A before B before C
        LEAST(
          tier_cap,
        ...
        ) OVER (PARTITION BY fid ORDER BY tord, co DESC, vel DESC) AS cum_need
      FROM v2_raw
    ),
    v2_ok AS (
