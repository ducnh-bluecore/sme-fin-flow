...
BEGIN
  -- Try as UUID first, then as fc_code
  BEGIN
  ...
  latest_snap AS (
    SELECT MAX(sp.snapshot_date) AS sd FROM inv_state_positions sp WHERE sp.tenant_id = v_tenant_uuid
  ),
  current_stock AS (
    SELECT COALESCE(SUM(sp.on_hand), 0)::int AS total_on_hand
    FROM inv_state_positions sp, latest_snap ls2
    WHERE sp.tenant_id = v_tenant_uuid AND sp.fc_id = v_fc_uuid AND sp.snapshot_date = ls2.sd
  ),
  sku_map AS (
  ...
  milestone_calc AS (
    SELECT json_agg(json_build_object(
      ...
      'actual_pct', CASE WHEN c.age_days >= (m->>'day')::int THEN c.sell_through_pct ELSE NULL END,
      'status', CASE
