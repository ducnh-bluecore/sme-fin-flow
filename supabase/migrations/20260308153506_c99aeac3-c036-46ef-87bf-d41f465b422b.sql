...
-- ============================================
-- Fix 5 Engine Bugs: Dedup, Reserve, Scarcity, Cumulative Push, BST mới
-- ============================================

-- ── Bug #1: Remove existing duplicates before adding constraint ──
DELETE FROM inv_allocation_recommendations a
USING inv_allocation_recommendations b
WHERE a.id > b.id
...
ALTER TABLE inv_allocation_recommendations
  ADD CONSTRAINT uq_alloc_run_fc_store UNIQUE (run_id, fc_id, store_id);

-- ── Bug #3: Update scarcity policy — add min_system_stock = 20 ──
UPDATE sem_allocation_policies
SET weights = weights || '{"min_system_stock": 20}'::jsonb
WHERE policy_type = 'SCARCITY';

-- ── Fix fn_allocation_engine: Bug #1 (dedup), #2 (reserve %), #3 (scarcity BST), #5 (V2 BST) ──
CREATE OR REPLACE FUNCTION fn_allocation_engine(
  p_tenant_id UUID,
  p_run_id UUID,
  ...
  IF p_run_type IN ('V1','both') THEN
    WITH v1_raw AS (
      SELECT f.id AS fid, f.fc_name, f.fc_code, f.is_core_hero, f.criticality_class, f.sku_count,
        ...
        cw.available AS cwt,
        -- Bug #2: Percentage-based reserve instead of fixed v_cw_reserve_min
        GREATEST(3,
      ...
      WHERE (COALESCE(p.oh,0)+COALESCE(p.it,0)) = 0
        ...
        -- Bug #3: BST mới bypass scarcity filter
      ...
      -- Bug #1: GROUP BY to eliminate duplicates from LEFT JOIN with multiple demand rows
  ...
  IF p_run_type IN ('V2','both') THEN
    WITH v2_raw AS (
      SELECT f.id AS fid,f.fc_name,f.fc_code,f.is_core_hero,f.criticality_class,
        s.id AS sid,s.store_name,s.store_code,s.tier,s.capacity,s.effective_capacity,s.tord,
        cw.available AS cwt,
        -- Bug #2: Percentage-based reserve
        GREATEST(3,
          CASE f.criticality_class
            WHEN 'CORE' THEN v_core_hero_min_per_sku * f.sku_count
            WHEN 'HERO' THEN GREATEST((v_core_hero_min_per_sku * f.sku_count * 0.7)::integer, FLOOR(cw.available * 0.15)::integer)
      ...
      LEFT JOIN tmp_dem d ON d.store_id=s.id AND d.fc_id=f.id
      LEFT JOIN tmp_soh soh ON soh.store_id=s.id
      -- Bug #5: BST mới bypass velocity filter
    ...
    v2_ranked AS (
      SELECT *,
        -- Bug #5: BST mới fallback to min_stock when velocity = 0
...
-- ── Fix fn_rebalance_engine: Bug #4 (cumulative push stock) ──
...
BEGIN
  ...
  -- Bug #4 FIX: Track cumulative push per FC
  push_cumulative AS (
    SELECT *,
