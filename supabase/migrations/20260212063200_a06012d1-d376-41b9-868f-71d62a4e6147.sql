...
BEGIN
  ...
  -- BETTER COGS: Use cdp_order_items.line_cogs for accurate COGS ratio
  SELECT 
  ...
  -- When order-level COGS is too low (<5% of revenue = incomplete), use estimated ratio
  ...
  -- Check if cost_price ≈ selling_price (data quality: basePrice used for both)
  ...
  -- If cost_price ≈ selling_price (ratio > 0.9), inventory is at RETAIL value
  -- Adjust to cost using COGS ratio
  ...
  -- Slow-moving: use inv_state_demand (aggregate by SKU to avoid duplicates)
  SELECT COALESCE(SUM(isp.on_hand * p.cost_price), 0)
  ...
  -- Also adjust slow_inventory if at retail price
  IF v_cost_sell_ratio > 0.9 AND v_cogs_ratio > 0 THEN
    v_slow_inventory := v_slow_inventory * v_cogs_ratio;
  END IF;

  -- 6. Locked Cash (use cost-adjusted inventory)
  ...
  -- DSO, DPO, DIO (use inventory_at_cost for DIO)
  ...
  -- Store inventory_at_cost as the reported value (more accurate)
  INSERT INTO central_metrics_snapshots (
    ...
    'v1.4', 'compute_central_metrics_snapshot',
