...
BEGIN
  ...
  v_select_cols := array_to_string(
    ARRAY(SELECT format(
      CASE 
        WHEN c IN ('raw_data', 'metadata', 'tags', 'extra', 'evidence', 'conditions', 'actions',
                   'request_payload', 'response_payload', 'execution_result', 'results',
                   'input_context', 'prediction_result', 'input_values', 'data') THEN
          'CASE WHEN r->%L IS NOT NULL THEN (r->%L)::jsonb ELSE NULL END'
        ELSE
      ...
      CASE 
        WHEN c IN ('raw_data', 'metadata', 'tags', 'extra', 'evidence', 'conditions', 'actions',
                   'request_payload', 'response_payload', 'execution_result', 'results',
                   'input_context', 'prediction_result', 'input_values', 'data') THEN 'jsonb'
        WHEN c IN ('tenant_id', 'order_id', 'customer_id', 'id', 'integration_id', 'gl_account_id') THEN 'uuid'
        WHEN c IN ('cost_price', 'selling_price', 'current_stock', 'quantity', 'unit_price', 'total_price',
                   ...
                   'line_revenue', 'original_price', 'gross_revenue') THEN 'numeric'
        WHEN c IN ('is_active', 'is_cod', 'is_first_order', 'is_verified', 'is_marketplace') THEN 'boolean'
        WHEN c IN ('order_number', 'total_orders', 'total_items', 'item_count', 'order_count', 'qty') THEN 'integer'
        WHEN c IN ('order_date', 'payment_date', 'refund_date', 'fulfilled_at', 'shipped_at', 'delivered_at',
                   ...
                   'spend_date', 'start_date', 'end_date', 'movement_date', 'snapshot_date',
                   'order_at') THEN 'timestamptz'
        ELSE 'text'
      END
