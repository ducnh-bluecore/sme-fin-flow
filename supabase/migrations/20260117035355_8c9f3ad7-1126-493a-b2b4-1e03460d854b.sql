...
-- Update function to support configurable period (default all-time for demo data)
CREATE OR REPLACE FUNCTION public.recalculate_product_metrics(
  p_tenant_id UUID,
  p_sku TEXT DEFAULT NULL,
  p_days_back INTEGER DEFAULT NULL -- NULL = all time
)
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  ELSE
    v_date_filter := '1970-01-01'::TIMESTAMP WITH TIME ZONE; -- All time
  END IF;

  -- Get aggregated data from external_order_items (SSOT)
  FOR v_record IN
    SELECT 
      eoi.tenant_id,
      eoi.sku,
  ...
  LOOP
    ...
    BEGIN
      -- Calculate period for daily averages
      v_period_days := GREATEST(EXTRACT(EPOCH FROM (NOW() - v_date_filter)) / 86400, 1);
      ...
      v_profit_per_unit := CASE 
        ...
        ELSE v_gross_margin 
      END;
      v_avg_daily_qty := v_record.total_qty / v_period_days;
