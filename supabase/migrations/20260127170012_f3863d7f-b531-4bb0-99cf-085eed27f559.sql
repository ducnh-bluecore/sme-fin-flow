-- Drop and recreate get_sku_profitability_by_date_range with correct column names
-- cdp_order_items: qty, line_revenue, line_cogs, product_id
-- cdp_orders: order_at, channel (no status column)
...
BEGIN
  RETURN QUERY
  SELECT
    COALESCE(p.code, coi.product_id, 'Unknown')::text AS sku,
    ...
    COALESCE(SUM(coi.line_revenue), 0) - COALESCE(SUM(coi.line_cogs), 0) AS gross_profit,
    CASE 
      WHEN COALESCE(SUM(coi.line_revenue), 0) > 0 
      THEN ROUND(
        ((COALESCE(SUM(coi.line_revenue), 0) - COALESCE(SUM(coi.line_cogs), 0)) / 
         COALESCE(SUM(coi.line_revenue), 0)) * 100, 2
      )
      ELSE 0
  ...
  GROUP BY COALESCE(p.code, coi.product_id, 'Unknown'), COALESCE(p.name, 'Unknown'), COALESCE(co.channel, 'Unknown')
