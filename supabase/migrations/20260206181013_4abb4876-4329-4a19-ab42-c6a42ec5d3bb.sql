...
-- Recreate compute function using timestamp range instead of ::date cast for index usage
...
BEGIN
  ...
  v_ts_end := (v_end + 1)::timestamptz; -- exclusive upper bound
  ...
  FROM (
    SELECT first_order_date::date AS fd, COUNT(*)::numeric AS val
    FROM cdp_customers WHERE tenant_id = p_tenant_id AND first_order_date >= v_start AND first_order_date <= v_end
