...
-- Fix cdp_evaluate_segments function - use correct column names
  ...
  LOOP
    ...
    IF v_segment.definition_json->>'source' = 'research_view' 
      ...
      LOOP
        ...
        -- Map field names to actual column conditions (using orders_180d as proxy for order count)
        CASE v_field
          WHEN 'orderCount' THEN
            CASE v_value
              WHEN '1' THEN v_where_clauses := array_append(v_where_clauses, 'orders_180d = 1');
              WHEN '2-5' THEN v_where_clauses := array_append(v_where_clauses, 'orders_180d BETWEEN 2 AND 5');
              WHEN '6-10' THEN v_where_clauses := array_append(v_where_clauses, 'orders_180d BETWEEN 6 AND 10');
              WHEN '>10' THEN v_where_clauses := array_append(v_where_clauses, 'orders_180d > 10');
              ELSE NULL;
            END CASE;
          WHEN 'totalSpend' THEN
            CASE v_value
              WHEN '<1tr' THEN v_where_clauses := array_append(v_where_clauses, 'net_revenue_180d < 1000000');
              WHEN '1-5tr' THEN v_where_clauses := array_append(v_where_clauses, 'net_revenue_180d BETWEEN 1000000 AND 5000000');
              WHEN '5-20tr' THEN v_where_clauses := array_append(v_where_clauses, 'net_revenue_180d BETWEEN 5000000 AND 20000000');
              WHEN '>20tr' THEN v_where_clauses := array_append(v_where_clauses, 'net_revenue_180d > 20000000');
              ELSE NULL;
            END CASE;
          WHEN 'aov' THEN
            CASE v_value
              WHEN '<200k' THEN v_where_clauses := array_append(v_where_clauses, 'aov_90d < 200000');
              WHEN '200k-500k' THEN v_where_clauses := array_append(v_where_clauses, 'aov_90d BETWEEN 200000 AND 500000');
              WHEN '500k-1tr' THEN v_where_clauses := array_append(v_where_clauses, 'aov_90d BETWEEN 500000 AND 1000000');
              WHEN '>1tr' THEN v_where_clauses := array_append(v_where_clauses, 'aov_90d > 1000000');
              ELSE NULL;
            END CASE;
