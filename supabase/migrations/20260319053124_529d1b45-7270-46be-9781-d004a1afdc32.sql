...
BEGIN
  ...
  WITH first_orders AS (
    SELECT
      tenant_id,
      customer_id,
      MIN(order_at::date) AS first_order_date
  ...
  FROM (
    SELECT 
      ...
      COUNT(DISTINCT CASE WHEN o.order_at::date = f.first_order_date THEN o.customer_id END) AS monthly_new,
      AVG(CASE WHEN o.order_at::date = f.first_order_date THEN o.net_revenue END) AS first_aov
    FROM public.cdp_orders o
    JOIN first_orders f ON f.tenant_id = o.tenant_id AND f.customer_id = o.customer_id
  ...
  FOR v_i IN 0..(p_horizon_months - 1) LOOP
    ...
    FOR v_cohort_row IN 
      ...
      FROM public.v_cdp_ltv_by_cohort
      WHERE tenant_id = p_tenant_id
    ...
    v_new_revenue := COALESCE(v_new_customers_avg, 0) * COALESCE(v_aov_first_order, 0);
    v_ads_revenue := p_ads_spend * COALESCE(v_roas, 0);
