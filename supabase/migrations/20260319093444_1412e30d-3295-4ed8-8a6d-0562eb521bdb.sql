...
BEGIN
  ...
  FROM (SELECT date_trunc('month', order_at)::date as m, SUM(net_revenue) as rev
    FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
      AND order_at >= (v_latest_order_month - interval '24 months') AND order_at < v_as_of_cutoff
    GROUP BY 1) sub
  ...
  WITH monthly_totals AS (
    ...
    FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
      AND order_at >= (v_latest_order_month - interval '4 months') AND order_at < v_as_of_cutoff
    GROUP BY 1
  ),
  new_counts AS (
  ...
  FROM (SELECT date_trunc('month', order_at)::date as m,
    ...
    SUM(CASE WHEN customer_name IS NULL AND (customer_phone IS NULL OR LENGTH(customer_phone)<10) AND customer_id IS NULL THEN net_revenue ELSE 0 END) as anon_rev
    FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
      AND order_at >= (v_latest_order_month - interval '2 months') AND order_at < v_as_of_cutoff
    GROUP BY 1) mt;
  ...
  WITH monthly_rev AS (
    SELECT date_trunc('month', order_at)::date as m, SUM(net_revenue) as rev
    FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
      AND order_at >= (v_latest_order_month - interval '5 months') AND order_at < v_as_of_cutoff
