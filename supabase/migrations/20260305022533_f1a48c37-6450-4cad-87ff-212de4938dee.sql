...
BEGIN
  ...
  SELECT
    ...
    -- days_behind: cap at 9999 to prevent integer overflow
    LEAST(
      CASE
        WHEN ws.cur_target IS NOT NULL AND ws.st_pct < ws.cur_target 
          THEN ((ws.cur_target - ws.st_pct) / GREATEST(ws.st_pct / GREATEST(ws.a_days, 1), 0.1))::int
        ELSE 0
      END,
      9999
    ),
