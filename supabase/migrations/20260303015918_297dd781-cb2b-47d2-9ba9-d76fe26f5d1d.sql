...
BEGIN
  -- Load size health rules if configured
  SELECT 
  ...
  -- ============================
  -- Size Health: Composite Score using sem_size_health_rules
  -- ============================
  -- Step 1: Build per-store×FC size detail with size curve
  ...
  SELECT 
    ...
    sp.on_hand,
    -- Is this a core size?
    CASE WHEN m.size_code = ANY(v_core_sizes) THEN true ELSE false END AS is_core,
    -- Ideal ratio from size curve (if available)
    COALESCE((sc.size_ratios ->> m.size_code)::numeric, 0) AS ideal_ratio
  FROM inv_state_positions sp
  ...
  -- Step 2: Aggregate into composite score per FC×Store
  ...
  SELECT 
    ...
    LEAST(100, GREATEST(0, (
      -- Component 1: Missing ratio (% of sizes present above threshold)
      v_w_missing * (COUNT(CASE WHEN on_hand > v_min_stock THEN 1 END)::numeric / NULLIF(COUNT(*), 0))
      -- Component 2: Core size presence
      + v_w_core * (
          CASE WHEN COUNT(CASE WHEN is_core THEN 1 END) = 0 THEN 1.0  -- no core sizes in range = skip
          ELSE COUNT(CASE WHEN is_core AND on_hand > v_min_stock THEN 1 END)::numeric 
               / NULLIF(COUNT(CASE WHEN is_core THEN 1 END), 0)
          END
        )
      -- Component 3: Curve deviation (lower deviation = higher score)
      + v_w_curve * (
          CASE WHEN SUM(ideal_ratio) = 0 THEN 1.0  -- no curve defined = skip
      ...
      -- Component 4: Depth score (% of sizes above shallow threshold)
