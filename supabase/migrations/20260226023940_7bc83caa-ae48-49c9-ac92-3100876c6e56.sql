...
BEGIN
  ...
  FROM inv_stores WHERE tenant_id = p_tenant_id AND is_active = TRUE
  ...
  FROM inv_state_demand d JOIN _retail_stores rs ON rs.store_id = d.store_id
  WHERE d.tenant_id = p_tenant_id;

  CREATE TEMP TABLE _sku_map ON COMMIT DROP AS
  SELECT m.fc_id, m.sku, m.size AS size_code
  FROM inv_sku_fc_mapping m WHERE m.tenant_id = p_tenant_id AND m.is_active = TRUE;

  CREATE TEMP TABLE _multi_size_fcs ON COMMIT DROP AS
  SELECT fc_id, COUNT(DISTINCT size_code) AS size_count
  FROM _sku_map WHERE size_code IS NOT NULL GROUP BY fc_id HAVING COUNT(DISTINCT size_code) >= 2;
  ...
  fc_stats AS (
    SELECT fc_id, AVG(doc) AS mean_doc, STDDEV_POP(doc) AS stddev_doc
  ...
  fc_locked AS (
    SELECT sd.fc_id,
      ...
      to_jsonb(ARRAY_AGG(DISTINCT sd.store_id) FILTER (WHERE sd.doc > fs.mean_doc * 1.5)) AS overstock_locs,
      to_jsonb(ARRAY_AGG(DISTINCT sd.store_id) FILTER (WHERE sd.doc < fs.mean_doc * 0.5 AND sd.doc < 14)) AS understock_locs
  ...
  SELECT p_tenant_id, p_as_of_date, fs.fc_id, ROUND(fs.stddev_doc::numeric, 2),
    COALESCE(fl.overstock_locs, '[]'::jsonb), COALESCE(fl.understock_locs, '[]'::jsonb), COALESCE(fl.locked_cash, 0)
  FROM fc_stats fs LEFT JOIN fc_locked fl ON fl.fc_id = fs.fc_id;
  GET DIAGNOSTICS v_idi_count = ROW_COUNT;

  -- ═══ SCS (kpi_size_completeness: style_id, sizes_present, sizes_total, size_completeness_score, status) ═══
  ...
  scs_calc AS (
    SELECT ss.style_id, ss.store_id, e.total_sizes, ss.present_sizes,
      ARRAY(SELECT unnest(e.all_sizes) EXCEPT SELECT unnest(ss.present_arr)) AS missing,
      ROUND((ss.present_sizes::numeric / NULLIF(e.total_sizes, 0)), 4) AS scs
  ...
  SELECT p_tenant_id, style_id, store_id, p_as_of_date, total_sizes, present_sizes, missing, scs,
    CASE WHEN scs < 0.3 THEN 'BROKEN' WHEN scs < 0.5 THEN 'AT_RISK' ELSE 'HEALTHY' END
  FROM scs_calc;
  ...
  WITH scs_data AS (
    SELECT style_id, store_id, size_completeness_score AS scs
  ...
  SELECT p_tenant_id, p_as_of_date, style_id, ROUND(curve_health_index::numeric, 4),
    CASE WHEN curve_health_index < 0.3 THEN 'CRITICAL' WHEN curve_health_index < 0.5 THEN 'HIGH'
         WHEN curve_health_index < 0.7 THEN 'MEDIUM' ELSE 'LOW' END FROM chi;
  ...
  FROM gap g LEFT JOIN _fc_price fp ON fp.fc_id = g.style_id WHERE CEIL(g.demand_28d) > g.total_stock;
  ...
  fc_totals AS (SELECT fc_id, SUM(total_qty) AS total_on_hand FROM fc_size_stock GROUP BY fc_id HAVING SUM(total_qty) > 0),
  deviation AS (
    SELECT ft.fc_id,
    ...
    GROUP BY ft.fc_id, msf.size_count, ft.total_on_hand
  )
  ...
  LEFT JOIN _fc_price fp ON fp.fc_id = fm.fc_id WHERE COALESCE(fv.total_velocity, 0) > 0;
    ...
    SELECT h.product_id, h.size_health_score,
      ...
      LEAST(100,
        ...
        + CASE WHEN COALESCE(fv.total_velocity, 0) > 0 AND COALESCE(fs.total_stock, 0) > 0 AND COALESCE(fs.total_stock, 0) / fv.total_velocity > 90 THEN 30
               ...
               WHEN COALESCE(fs.total_stock, 0) > 0 AND COALESCE(fv.total_velocity, 0) <= 0 THEN 30 ELSE 0 END
      ) AS raw_risk
  ...
  SELECT p_tenant_id, product_id, p_as_of_date, raw_risk,
  ...
  store_fc_total AS (SELECT store_id, fc_id, SUM(qty) AS total_qty FROM store_fc_size GROUP BY store_id, fc_id HAVING SUM(qty) > 0),
  store_deviation AS (
  ...
  LEFT JOIN _fc_stock fs ON fs.fc_id = sh.product_id LEFT JOIN _fc_price fp ON fp.fc_id = sh.product_id
  ...
  LEFT JOIN _fc_stock fs ON fs.fc_id = md.product_id LEFT JOIN _fc_price fp ON fp.fc_id = md.product_id
  ...
  SELECT p_tenant_id, sh.product_id, p_as_of_date, 'size_intelligence',
    ...
    jsonb_build_object(
      'health', jsonb_build_object('score', sh.size_health_score, 'state', sh.curve_state, 'core_missing', sh.core_size_missing),
      ...
      'markdown_risk', CASE WHEN md.product_id IS NOT NULL THEN jsonb_build_object('score', md.markdown_risk_score, 'eta_days', md.markdown_eta_days) END,
