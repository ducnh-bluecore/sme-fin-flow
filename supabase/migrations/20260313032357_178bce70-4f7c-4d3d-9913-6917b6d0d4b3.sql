...
CREATE OR REPLACE FUNCTION public.compute_central_metrics_snapshot(p_tenant_id uuid, p_start_date date DEFAULT NULL::date, p_end_date date DEFAULT NULL::date)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ★ FIX: Inline schema routing instead of calling set_tenant_search_path
  -- (SET LOCAL inside SECURITY DEFINER sub-function loses scope on return)
  SELECT 'tenant_' || slug, COALESCE(schema_provisioned, false)
  INTO v_tenant_schema, v_provisioned
  FROM public.tenants WHERE id = p_tenant_id AND is_active = true;

  IF v_provisioned AND EXISTS (
    SELECT 1 FROM information_schema.schemata WHERE schema_name = v_tenant_schema
  ) THEN
    EXECUTE format('SET LOCAL search_path TO %I, public', v_tenant_schema);
    RAISE NOTICE '[FDP] Routed to schema: %', v_tenant_schema;
  ELSE
    SET LOCAL search_path TO public;
    v_tenant_schema := 'public';
    RAISE NOTICE '[FDP] Using public schema';
  END IF;
  ...
  -- 1. REVENUE from cdp_orders (schema-aware via search_path)
  SELECT 
  ...
  BEGIN
    SELECT AVG(cost_price / NULLIF(selling_price, 0))
    INTO v_cost_sell_ratio
    FROM products 
    WHERE tenant_id = p_tenant_id AND cost_price > 0 AND selling_price > 0;
  EXCEPTION WHEN undefined_table THEN
    v_cost_sell_ratio := 0;
  END;

  IF v_cost_sell_ratio > 0.9 AND v_cogs_ratio > 0 THEN
    v_inventory_at_cost := v_inventory_value * v_cogs_ratio;
  ELSE
    v_inventory_at_cost := v_inventory_value;
  END IF;
  ...
  BEGIN
    ...
    LEFT JOIN (
      ...
      FROM inv_state_demand WHERE tenant_id = p_tenant_id GROUP BY sku, tenant_id
  ...
  IF v_cost_sell_ratio > 0.9 AND v_cogs_ratio > 0 THEN
    v_slow_inventory := v_slow_inventory * v_cogs_ratio;
  END IF;

  -- 6. Locked Cash
  v_locked_inventory := v_inventory_at_cost;

  BEGIN
    SELECT COALESCE(SUM(amount), 0) INTO v_locked_ads
    FROM marketing_expenses WHERE tenant_id = p_tenant_id AND expense_date > CURRENT_DATE - INTERVAL '14 days';
  EXCEPTION WHEN undefined_table THEN v_locked_ads := 0;
  END;

  BEGIN
    SELECT COALESCE(SUM(total_amount - COALESCE(paid_amount,0)), 0) INTO v_locked_ops
    FROM bills WHERE tenant_id = p_tenant_id AND status IN ('pending','partial','overdue')
      AND expense_category IN ('shipping','logistics','fulfillment','delivery');
  EXCEPTION WHEN undefined_table THEN v_locked_ops := 0;
  END;
  ...
  BEGIN
    ...
    FROM expenses WHERE tenant_id = p_tenant_id AND expense_date BETWEEN v_start_date AND v_end_date;
  EXCEPTION WHEN undefined_table THEN
    v_total_opex := 0; v_variable_costs := 0; v_depreciation := 0;
  END;

  -- 8. Marketing
  BEGIN
    SELECT COALESCE(SUM(actual_cost), 0), COALESCE(SUM(total_revenue), 0)
    INTO v_marketing_spend, v_marketing_revenue
    FROM promotion_campaigns
    WHERE tenant_id = p_tenant_id AND created_at >= v_start_date AND created_at <= v_end_date
      AND (status IS NULL OR status != 'cancelled');
  EXCEPTION WHEN undefined_table THEN
    v_marketing_spend := 0; v_marketing_revenue := 0;
  END;
  ...
  WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled', 'refunded')
    AND customer_id IN (
      ...
      WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled', 'refunded')
      GROUP BY customer_id HAVING MIN(order_at::date) BETWEEN v_start_date AND v_end_date
    )
    AND order_at::date BETWEEN v_start_date AND v_end_date;
  ...
  SELECT COALESCE(AVG(cust_revenue) * AVG(cust_orders), 0) INTO v_ltv
  FROM (
    SELECT customer_id, SUM(net_revenue) AS cust_revenue, COUNT(*) AS cust_orders
    FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled', 'refunded')
  ...
  v_dio := CASE WHEN v_daily_cogs > 0 THEN v_inventory_at_cost / v_daily_cogs ELSE 0 END;
  ...
  -- Always write to public schema
  INSERT INTO public.central_metrics_snapshots (
    ...
    v_inventory_at_cost, v_slow_inventory,
    ...
    v_start_date, v_end_date, 'compute_central_metrics_snapshot_v1.6', v_duration_ms,
    'v1.6-inline-schema', 'compute_central_metrics_snapshot',
