-- Data Reset Script: Truncate all operational/governance data
-- This is a one-time cleanup, not a schema migration

-- 1) Stop ML writes if you have background jobs (optional safety)
UPDATE public.tenant_ml_settings
SET ml_enabled = false
WHERE tenant_id IS NOT NULL;

-- 2) Truncate operational / governance data first
-- (Ledger, allocations, suggestions, exceptions, snapshots, logs, approvals, audit)
...
BEGIN
  ...
  -- Decision snapshots (truth layer)
  ...
  -- 3) Evidence layer last (orders/invoices/bank/customers/products)
  ...
  -- Reference data (optional reset; keep if you want stable masters)
