...
BEGIN
  BEGIN
    ...
    PERFORM 1 FROM public.inv_family_codes WHERE id = v_fc_uuid AND tenant_id = v_tenant_uuid;
  ...
  latest_per_store AS (
    ...
    JOIN public.inv_sku_fc_mapping m
      ON m.tenant_id = sp.tenant_id
     AND m.sku = sp.sku
    WHERE sp.tenant_id = v_tenant_uuid
  ...
  current_stock AS (
    ...
    JOIN public.inv_sku_fc_mapping m
      ON m.tenant_id = sp.tenant_id
     AND m.sku = sp.sku
    JOIN latest_per_store lps
      ON lps.store_id = sp.store_id
     AND lps.max_sd = sp.snapshot_date
    WHERE sp.tenant_id = v_tenant_uuid
      AND m.fc_id = v_fc_uuid
  ),
  sku_map AS (
    ...
    WHERE m.tenant_id = v_tenant_uuid
      AND m.fc_id = v_fc_uuid
  ),
  sales_data AS (
  ...
  milestone_calc AS (
    SELECT json_agg(
      json_build_object(
        'day', mwb.day,
        'target_pct', mwb.target_pct,
        'actual_pct', CASE
          WHEN c.age_days >= mwb.stage_start_day AND c.age_days <= mwb.day THEN c.sell_through_pct
          WHEN c.age_days > mwb.day THEN c.sell_through_pct
          ELSE NULL
        END,
        'status', CASE
          WHEN c.age_days < mwb.stage_start_day THEN 'upcoming'
          WHEN c.sell_through_pct >= mwb.target_pct THEN 'ahead'
          WHEN c.sell_through_pct >= mwb.target_pct * 0.8 THEN 'on_track'
          ELSE 'behind'
        END
      )
      ORDER BY mwb.day
    ) AS ms
    FROM milestone_with_bounds mwb
