...
-- Drop existing function first (it had different defaults)
DROP FUNCTION IF EXISTS public.cdp_run_daily_build(uuid, date);

-- Recreate with batched equity
...
BEGIN
  ...
  -- Step 3: Build customer equity (BATCHED - 10K customers per batch)
  v_equity_result := cdp_build_customer_equity_batched(p_tenant_id, p_as_of_date, 10000);
