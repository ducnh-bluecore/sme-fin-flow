...
DROP FUNCTION IF EXISTS public.tenant_lookup_order_ids(uuid, text[]);

CREATE FUNCTION public.tenant_lookup_order_ids(
  p_tenant_id uuid,
  p_order_keys text[]
)
RETURNS TABLE(order_uuid uuid, order_key text)
LANGUAGE plpgsql
SECURITY DEFINER
