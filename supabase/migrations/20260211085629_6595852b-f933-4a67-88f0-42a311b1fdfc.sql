...
BEGIN
  ...
  SELECT
    ...
    co.channel,
    co.order_at,
    coi.qty::numeric AS quantity,
    coi.unit_price,
    coi.line_revenue,
    coi.line_cogs,
    coi.line_margin,
    co.gross_revenue AS order_gross_revenue,
    COALESCE(co.platform_fee, 0)::numeric AS order_platform_fee,
    COALESCE(co.shipping_fee, 0)::numeric AS order_shipping_fee,
    COALESCE(co.other_fees, 0)::numeric AS order_other_fees,
