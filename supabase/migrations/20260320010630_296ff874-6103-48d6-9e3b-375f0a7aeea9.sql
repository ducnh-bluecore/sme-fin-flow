...
BEGIN
  ...
  -- Pre-aggregate monthly data in one pass (key optimization)
  CREATE TEMP TABLE IF NOT EXISTS _fc_monthly ON COMMIT DROP AS
  SELECT 
  ...
  -- Overall YoY from pre-aggregated data
  SELECT COALESCE(
  ...
  -- LY monthly rev from pre-aggregated data
  v_ly_monthly_rev := '{}'::jsonb;
  FOR ly_rec IN
  ...
  -- YoY growth factors from pre-aggregated data
  v_yoy_growth_factors := '{}'::jsonb;
  FOR yoy_rec IN
    WITH rolling_avg AS (
  ...
  -- Seasonal multipliers from pre-aggregated data
  v_seasonal_multipliers := '{}'::jsonb;
  FOR seasonal_rec IN
    WITH with_stats AS (
  ...
  -- Base revenue from pre-aggregated data (last 4 months)
  ...
  -- Recent months for weighted average (from pre-aggregated data)
  ...
  -- Monthly decay from pre-aggregated data
  WITH mom AS (
  ...
  -- Cohort breakdown (still needs cdp_orders for customer-level data)
  v_cohort_json := '[]'::jsonb;
  FOR cohort_rec IN
