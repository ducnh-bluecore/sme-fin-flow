...
BEGIN
  ...
  -- 3. RETAIL AR: orders in-transit/pending settlement (last 60 days only)
  ...
  -- Overdue AR: orders > 14 days still not settled
  SELECT COALESCE(SUM(net_revenue), 0)
  INTO v_overdue_ar
  FROM cdp_orders
  WHERE tenant_id = p_tenant_id
  ...
  -- 5. Inventory from inv_state_positions (NOT products.current_stock)
  ...
  -- Slow-moving inventory: SKUs with no recent sales (>90 days)
  SELECT COALESCE(SUM(isp.on_hand * p.cost_price), 0)
  INTO v_slow_inventory
  FROM inv_state_positions isp
  JOIN products p ON p.sku = isp.sku AND p.tenant_id = isp.tenant_id
  WHERE isp.tenant_id = p_tenant_id AND isp.on_hand > 0
    AND (p.last_sale_date IS NULL OR p.last_sale_date < CURRENT_DATE - INTERVAL '90 days');
  ...
  -- 10. Derived metrics
  v_contribution_margin := v_gross_profit - v_variable_costs - v_marketing_spend;
  ...
  v_dio := CASE WHEN v_annual_cogs > 0 THEN (v_inventory_value / v_annual_cogs) * 365 ELSE 0 END;
  v_ccc := v_dso + v_dio - v_dpo;

  v_cash_runway_months := CASE 
    WHEN (v_total_opex + v_variable_costs) > 0 THEN 
      v_cash_today / ((v_total_opex + v_variable_costs) / GREATEST(v_days_in_period / 30.0, 1))
    ELSE 0 
  END;
  ...
  INSERT INTO finance_truth_snapshots (
    id, tenant_id, snapshot_date, period_start, period_end,
    net_revenue, gross_profit, total_cogs,
    cash_today, cash_7d_forecast, cash_runway_months,
    total_ar, overdue_ar, total_ap, overdue_ap,
    ar_current, ar_30d, ar_60d, ar_90d,
    total_inventory_value, slow_moving_inventory,
    locked_cash_inventory, locked_cash_ads, locked_cash_ops,
    locked_cash_platform, locked_cash_total,
    total_opex, variable_costs, depreciation,
    marketing_spend, marketing_revenue, marketing_roas,
    contribution_margin, ebitda,
    gross_margin_pct, contribution_margin_pct, ebitda_margin_pct,
    dso, dpo, dio, ccc,
    total_orders, total_customers, avg_order_value,
    new_customers, repeat_customers, repeat_customer_rate,
    cac, ltv, ltv_cac_ratio,
    computation_time_ms
  ) VALUES (
    v_snapshot_id, p_tenant_id, CURRENT_DATE, v_start_date, v_end_date,
    v_net_revenue, v_gross_profit, v_total_cogs,
    ...
    v_locked_inventory, v_locked_ads, v_locked_ops,
    v_locked_platform, v_locked_total,
    v_total_opex, v_variable_costs, v_depreciation,
    v_marketing_spend, v_marketing_revenue, v_marketing_roas,
    v_contribution_margin, v_ebitda,
    v_gross_margin_pct, v_contribution_margin_pct, v_ebitda_margin_pct,
    v_dso, v_dpo, v_dio, v_ccc,
    v_total_orders, v_total_customers, v_avg_order_value,
    v_new_customers, v_repeat_customers, v_repeat_customer_rate,
