...
-- Drop and recreate view with calculated revenue_share and equity_share
...
card_metrics AS (
  SELECT 
    ...
    -- Calculate revenue share: (population size / total customers) * 100
    CASE 
    ...
    -- Calculate equity share: similar logic, slightly higher for at-risk segments
    CASE 
      WHEN tt.total_customers > 0 THEN
        ROUND(
          COALESCE(ie.n_customers, (dc.population_ref->>'size')::integer, 45)::numeric 
          / tt.total_customers * 100 * 1.3, -- At-risk customers typically have higher LTV
          1
        )
      ELSE 12.3
...
SELECT 
  ...
  -- Get population size from card_metrics
