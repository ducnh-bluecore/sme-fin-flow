...
-- Master PL/pgSQL function: compute ALL inventory KPIs server-side
-- Replaces 1300 lines of Edge Function JS logic
-- No CPU timeout since PostgreSQL has no 2s limit
...
BEGIN
  ...
  -- ═══════════════════════════════════════════
  -- STEP 0: Prepare temp tables for shared data
  -- ═══════════════════════════════════════════
  ...
  -- All positions (retail only, on_hand > 0)
  ...
  GROUP BY fc_id
  HAVING COUNT(DISTINCT size_code) >= 2;

  -- FC expected sizes
  CREATE TEMP TABLE _fc_sizes ON COMMIT DROP AS
  SELECT fc_id, size_code
  FROM _sku_map
  WHERE size_code IS NOT NULL
  GROUP BY fc_id, size_code;

  -- Avg unit prices per SKU
  CREATE TEMP TABLE _prices ON COMMIT DROP AS
  SELECT sku, avg_unit_price
  FROM v_inv_avg_unit_price
  WHERE tenant_id = p_tenant_id AND avg_unit_price > 0;

  -- FC avg price
  CREATE TEMP TABLE _fc_price ON COMMIT DROP AS
  SELECT m.fc_id, AVG(pr.avg_unit_price) AS avg_price
  FROM _sku_map m
  JOIN _prices pr ON pr.sku = m.sku
  GROUP BY m.fc_id;
  ...
  -- ═══════════════════════════════════════════
  -- STEP 1: IDI — Inventory Distortion Index
  -- ═══════════════════════════════════════════
  ...
  WITH store_doc AS (
    SELECT
      ...
      CASE
        WHEN COALESCE(d.avg_daily_sales, 0) > 0.01
        THEN LEAST(180, p.on_hand / d.avg_daily_sales)
        ELSE CASE WHEN p.on_hand > 0 THEN 180 ELSE 0 END
      END AS doc
    FROM _positions p
  ...
  fc_locked AS (
    ...
    FROM store_doc sd
    JOIN fc_stats fs ON fs.fc_id = sd.fc_id
    GROUP BY sd.fc_id
  )
  ...
  SELECT
    ...
    p_as_of_date,
    fs.fc_id,
    ROUND(fs.stddev_doc::numeric, 2),
    COALESCE(fl.overstock_locs, '{}'),
    COALESCE(fl.understock_locs, '{}'),
    COALESCE(fl.locked_cash, 0)
  FROM fc_stats fs
  LEFT JOIN fc_locked fl ON fl.fc_id = fs.fc_id;

  GET DIAGNOSTICS v_idi_count = ROW_COUNT;

  -- ═══════════════════════════════════════════
  -- STEP 2: SCS — Size Completeness Score
  -- ═══════════════════════════════════════════

  DELETE FROM kpi_size_completeness WHERE tenant_id = p_tenant_id AND as_of_date = p_as_of_date;

  -- Build sku -> (fc_id, size) lookup and resolve positions
  WITH pos_with_size AS (
    SELECT
      ...
      m.size_code
    FROM _positions p
    JOIN _sku_map m ON m.sku = p.sku
    JOIN _multi_size_fcs msf ON msf.fc_id = m.fc_id
    WHERE m.size_code IS NOT NULL
  ),
  store_style_sizes AS (
    SELECT store_id, style_id, COUNT(DISTINCT size_code) AS present_sizes,
      ARRAY_AGG(DISTINCT size_code) AS present_arr
    FROM pos_with_size
    GROUP BY store_id, style_id
  ),
  expected AS (
    SELECT fc_id, COUNT(DISTINCT size_code) AS total_sizes,
      ARRAY_AGG(DISTINCT size_code) AS all_sizes
    FROM _fc_sizes
    WHERE fc_id IN (SELECT fc_id FROM _multi_size_fcs)
    GROUP BY fc_id
  )
  ...
  FROM store_style_sizes ss
  JOIN expected e ON e.fc_id = ss.style_id;

  GET DIAGNOSTICS v_scs_count = ROW_COUNT;

  -- ═══════════════════════════════════════════
  -- STEP 3: CHI — Curve Health Index
  -- ═══════════════════════════════════════════
  ...
  weighted AS (
    SELECT
      s.style_id,
      s.scs,
      COALESCE(d.avg_daily_sales, 0) AS weight
    FROM scs_data s
    LEFT JOIN _demand d ON d.store_id = s.store_id AND d.fc_id = s.style_id
  ),
  chi AS (
    SELECT
      style_id,
      CASE WHEN SUM(weight) > 0
        THEN SUM(scs * weight) / SUM(weight)
        ELSE AVG(scs)
      END AS curve_health_index
    FROM weighted
    GROUP BY style_id
  )
  ...
  SELECT
    ...
    p_as_of_date,
    style_id,
    ROUND(curve_health_index::numeric, 4),
    CASE
      WHEN curve_health_index < 0.3 THEN 'CRITICAL'
      WHEN curve_health_index < 0.5 THEN 'HIGH'
      WHEN curve_health_index < 0.7 THEN 'MEDIUM'
      ELSE 'LOW'
    END
  FROM chi;
  ...
  WITH gap AS (
    SELECT
      COALESCE(fs.fc_id, fv.fc_id) AS style_id,
      COALESCE(fs.total_stock, 0) AS total_stock,
      COALESCE(fv.total_velocity, 0) * 28 AS demand_28d
    FROM _fc_stock fs
    FULL OUTER JOIN _fc_velocity fv ON fv.fc_id = fs.fc_id
  ...
  SELECT
    ...
    p_as_of_date,
    g.style_id,
    FLOOR(g.total_stock * 0.7)::int,
    GREATEST(0, CEIL(g.demand_28d) - g.total_stock)::int,
    GREATEST(0, CEIL(g.demand_28d) - FLOOR(g.total_stock * 0.7))::int,
    GREATEST(0, CEIL(g.demand_28d) - g.total_stock) * COALESCE(fp.avg_price, 250000)
  FROM gap g
  LEFT JOIN _fc_price fp ON fp.fc_id = g.style_id
  WHERE CEIL(g.demand_28d) > g.total_stock;

  GET DIAGNOSTICS v_gap_count = ROW_COUNT;

  -- ═══════════════════════════════════════════
  -- STEP 5: Size Health Score (network-wide)
  -- ═══════════════════════════════════════════
  ...
  fc_totals AS (
    SELECT fc_id, SUM(total_qty) AS total_on_hand
    FROM fc_size_stock
    GROUP BY fc_id
  ...
  deviation AS (
    SELECT
      ...
      SUM(
        ABS(
          (1.0 / msf.size_count) - (COALESCE(fss.total_qty, 0)::numeric / ft.total_on_hand)
        ) * CASE WHEN fs.size_code IN ('M', 'L') THEN 1.5 ELSE 1.0 END
      ) AS deviation_sum,
      BOOL_OR(
        fs.size_code IN ('M', 'L') AND COALESCE(fss.total_qty, 0) <= 0
      ) AS core_missing,
      COUNT(*) FILTER (WHERE COALESCE(fss.total_qty, 0) > 0 AND COALESCE(fss.total_qty, 0) < 2) AS shallow_count
    FROM fc_totals ft
    JOIN _multi_size_fcs msf ON msf.fc_id = ft.fc_id
  ...
  fc_missing AS (
    ...
    FROM _fc_sizes fs
    JOIN _multi_size_fcs msf ON msf.fc_id = fs.fc_id
  ...
  SELECT
    ...
    p_as_of_date,
    CEIL(
      COALESCE(fv.total_velocity, 0) * 28 *
      LEAST(0.8, fm.missing_count::numeric / fm.size_count + (fm.shallow_count::numeric / fm.size_count * 0.3))
    )::int,
    CEIL(
      COALESCE(fv.total_velocity, 0) * 28 *
      LEAST(0.8, fm.missing_count::numeric / fm.size_count + (fm.shallow_count::numeric / fm.size_count * 0.3))
    ...
    CASE
      WHEN fm.core_missing THEN 'core_missing'
      WHEN fm.shallow_count > 0 THEN 'shallow'
      ELSE 'imbalance'
    END
  FROM fc_missing fm
  JOIN _fc_velocity fv ON fv.fc_id = fm.fc_id
  ...
  fc_woc AS (
    SELECT fc_id, AVG(weeks_of_cover) AS avg_woc
    FROM _positions
  ...
  risk_calc AS (
    SELECT
      h.product_id,
      h.size_health_score,
      COALESCE(fv.total_velocity, 0) AS velocity,
      COALESCE(fs.total_stock, 0) AS stock,
      COALESCE(fw.avg_woc, 0) AS avg_woc,
      -- Size health factor
      CASE WHEN h.size_health_score < 40 THEN 40
           WHEN h.size_health_score < 60 THEN 20
           ELSE 0 END
      -- Age factor
      + CASE WHEN COALESCE(fw.avg_woc, 0) > 12 THEN 30
             WHEN COALESCE(fw.avg_woc, 0) > 8 THEN 15
             ELSE 0 END
      -- Velocity factor
      + CASE
          WHEN COALESCE(fv.total_velocity, 0) > 0 AND COALESCE(fs.total_stock, 0) > 0 AND
               COALESCE(fs.total_stock, 0) / fv.total_velocity > 90 THEN 30
          WHEN COALESCE(fv.total_velocity, 0) > 0 AND COALESCE(fs.total_stock, 0) > 0 AND
               COALESCE(fs.total_stock, 0) / fv.total_velocity > 60 THEN 15
          WHEN COALESCE(fs.total_stock, 0) > 0 AND COALESCE(fv.total_velocity, 0) <= 0 THEN 30
          ELSE 0 END
      AS raw_risk
    FROM health h
    LEFT JOIN _fc_velocity fv ON fv.fc_id = h.product_id
    LEFT JOIN _fc_stock fs ON fs.fc_id = h.product_id
    LEFT JOIN fc_woc fw ON fw.fc_id = h.product_id
  )
  ...
  SELECT
    ...
    LEAST(100, raw_risk),
    CASE
      WHEN raw_risk >= 60 AND velocity > 0 THEN LEAST(90, GREATEST(7, ROUND(stock / GREATEST(velocity, 0.1) / 2)))::int
      WHEN raw_risk >= 40 AND velocity > 0 THEN LEAST(120, ROUND(stock / velocity))::int
      WHEN raw_risk >= 40 THEN 60
      ELSE NULL
    END,
    CONCAT_WS(' + ',
  ...
  store_fc_total AS (
    SELECT store_id, fc_id, SUM(qty) AS total_qty
    FROM store_fc_size
  ...
  store_deviation AS (
    SELECT
      ...
      msf.size_count,
      SUM(
        ABS((1.0 / msf.size_count) - (COALESCE(sfs.qty, 0)::numeric / sft.total_qty))
        * CASE WHEN fs.size_code IN ('M', 'L') THEN 1.5 ELSE 1.0 END
      ) AS deviation_sum,
    ...
    FROM store_fc_total sft
    JOIN _multi_size_fcs msf ON msf.fc_id = sft.fc_id
  ...
  SELECT
    ...
    CASE WHEN COALESCE(fv.total_velocity, 0) > 0
      THEN LEAST(180, ROUND(COALESCE(fs.total_stock, 0) / fv.total_velocity))::int
      ELSE 180 END,
  ...
  WHERE md.tenant_id = p_tenant_id
    AND md.as_of_date = p_as_of_date
    AND md.markdown_risk_score >= 60
  ...
  SELECT
    ...
    'size_intelligence',
    CASE
      WHEN sh.curve_state = 'broken' AND COALESCE(md.markdown_risk_score, 0) >= 60 THEN 'critical'
      WHEN sh.curve_state = 'broken' THEN 'high'
      ELSE 'medium'
    END,
    CONCAT_WS(' | ',
  ...
  -- Note: Size Transfers (Step 12) is complex store-to-store matching.
  -- For now, skip in PL/pgSQL and handle in a separate simpler function if needed.
  -- The key heavy computations (Steps 1-11) are done.
  ...
  RETURN jsonb_build_object(
    ...
    'chi', v_chi_count,
    'gap', v_gap_count,
    'size_health', v_size_health_count,
    'lost_revenue', v_lost_revenue_count,
    'markdown_risk', v_markdown_risk_count,
    'store_health', v_store_health_count,
    'cash_lock', v_cash_lock_count,
    'margin_leak', v_margin_leak_count,
