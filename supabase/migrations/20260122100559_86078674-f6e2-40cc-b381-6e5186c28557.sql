-- Step 2: Create function to detect real alerts from actual business data
...
BEGIN
  ...
  SELECT 
    ...
    pm.sku || ': Margin chỉ ' || ROUND(COALESCE(pm.gross_margin_percent, 0)::numeric, 1) || '%. ' ||
  ...
  SELECT 
    ...
    'Khách: ' || COALESCE(i.customer_name, 'N/A') || '. Quá hạn ' || 
  ...
  SELECT 
    ...
    'Tổng số dư: ' || total_cash::text || ' VND. Cần theo dõi dòng tiền.',
    'Cash Position',
    'cash',
    total_cash,
    50000000,
  ...
  -- 4. PENDING BILLS (Unpaid vendor bills) from bills
  INSERT INTO alert_instances (
    tenant_id, alert_type, category, severity, title, message,
    object_name, object_type, current_value, threshold_value,
    impact_amount, status, created_at
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE 'info'
    END,
    'Hóa đơn NCC cần thanh toán: ' || b.bill_number,
    'Nhà cung cấp: ' || COALESCE(v.name, 'N/A') || '. Hạn: ' || b.due_date::text ||
    '. Còn nợ: ' || (b.total_amount - COALESCE(b.paid_amount, 0))::text,
    b.bill_number,
    'bill',
  ...
  SELECT 
    ...
    'Kênh ' || channel || ' có ' || return_count || ' đơn hoàn/' || total_count || ' đơn (' ||
    ROUND(return_rate::numeric, 1) || '%) trong 30 ngày qua.',
    channel,
    'channel',
    return_rate,
    5,
