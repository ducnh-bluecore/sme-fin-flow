...
-- Fix compute_inventory_kpi_all: remap size health INSERT to match actual table schema
...
BEGIN
  ...
  JOIN products p ON p.sku = m.sku AND p.tenant_id = m.tenant_id
  ...
  -- Size Health using ACTUAL table columns
  ...
  WHERE sp.tenant_id = p_tenant_id AND sp.snapshot_date = p_as_of_date AND m.is_active = true;
  ...
  INSERT INTO state_size_health_daily (
    ...
    size_health_score, curve_state, deviation_score, core_size_missing, shallow_depth_count
  )
  SELECT 
    v_tid, d.fc_id::text, d.store_id, p_as_of_date,
    ...
    COUNT(DISTINCT d.size_code) FILTER (WHERE d.on_hand > 0 AND d.on_hand <= v_shallow)
  FROM _size_detail d
  GROUP BY d.fc_id, d.store_id;
  ...
  SELECT v_tid, fc_id::text, p_as_of_date,
    ...
    CASE WHEN daily_vel = 0 THEN 'critical' WHEN total_on_hand / NULLIF(daily_vel, 0) > 120 THEN 'high' WHEN total_on_hand / NULLIF(daily_vel, 0) > 60 THEN 'medium' ELSE 'low' END,
    CASE WHEN daily_vel = 0 THEN 50 WHEN total_on_hand / NULLIF(daily_vel, 0) > 120 THEN 30 WHEN total_on_hand / NULLIF(daily_vel, 0) > 60 THEN 15 ELSE 0 END,
    CASE WHEN daily_vel = 0 THEN total_on_hand * cost ELSE GREATEST(0, total_on_hand - daily_vel * 45) * cost * 0.3 END
  FROM _inv_agg WHERE total_on_hand > 0;
  ...
  RETURN jsonb_build_object('success', true, 'as_of_date', p_as_of_date,
    'lost_revenue_rows', v_lr, 'cash_lock_rows', v_cl, 'margin_leak_rows', v_ml,
    'size_health_rows', v_sz, 'markdown_risk_rows', v_md, 'evidence_rows', v_ev,
