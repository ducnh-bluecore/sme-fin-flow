...
-- Update fn_allocation_engine to use SEM allocation policies + SKU criticality
...
BEGIN
  ...
  -- ── Filter non-fashion items + enrich with criticality ──
  ...
  IF p_run_type IN ('V1','both') THEN
    WITH v1_raw AS (
      ...
      WHERE cw.available>GREATEST(v_cw_reserve_min,CASE WHEN f.is_core_hero THEN v_core_hero_min_per_sku*f.sku_count ELSE 0 END)
        ...
        -- SCARCITY: if system total < 40 and policy active, only allow tiers with weight > 0
    ...
    SELECT p_tenant_id,p_run_id,fid,COALESCE(fc_name,fc_code,''),sid,COALESCE(store_name,store_code,''),NULL,aq::integer,cs,
      ...
      format('V1: FC %s (%s), CH %s (tier %s, w=%.1f) min %s, có %s, bổ sung %s',COALESCE(fc_name,fc_code),criticality_class,store_name,tier,tier_weight,mq,cs,aq),NULL
