...
-- Fix fn_clearance_candidates: cast TEXT↔UUID mismatches
...
-- Fix fn_evidence_pack_by_fc: cast TEXT↔UUID for kpi_size_completeness.style_id
...
BEGIN
  ...
  -- 2. Count stores and missing sizes per size (style_id is TEXT)
  WITH store_missing AS (
    SELECT sc.store_id, sc.missing_sizes
    FROM kpi_size_completeness sc
    WHERE sc.style_id = p_fc_id::text
