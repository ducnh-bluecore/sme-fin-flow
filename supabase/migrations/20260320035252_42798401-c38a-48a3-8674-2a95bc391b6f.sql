...
-- Fix compute_size_transfers to use actual snapshot date for delete too
CREATE OR REPLACE FUNCTION public.compute_size_transfers(p_tenant_id uuid, p_as_of_date date DEFAULT CURRENT_DATE)
RETURNS jsonb
LANGUAGE plpgsql
VOLATILE
...
BEGIN
  ...
  -- Delete old transfers for this tenant (all dates, since we regenerate)
  DELETE FROM state_size_transfer_daily 
  WHERE tenant_id = p_tenant_id::text;

  CREATE TEMP TABLE _fc_prices_t ON COMMIT DROP AS
  SELECT m.fc_id, MAX(p.selling_price) AS selling_price
  FROM inv_sku_fc_mapping m
  JOIN products p ON p.sku = m.sku AND p.tenant_id = m.tenant_id
  WHERE m.tenant_id = p_tenant_id AND m.is_active = true AND p.selling_price > 0
  GROUP BY m.fc_id;

  CREATE TEMP TABLE _multi_size_fcs_t ON COMMIT DROP AS
  SELECT fc_id
  ...
  WITH latest_inv AS (
    ...
    JOIN inv_sku_fc_mapping m ON m.sku = isp.sku AND m.tenant_id = isp.tenant_id AND m.is_active = true
    JOIN inv_stores s ON s.id = isp.store_id AND s.is_transfer_eligible = true AND s.location_type = 'store'
    JOIN _multi_size_fcs_t msf ON msf.fc_id = isp.fc_id
    WHERE isp.tenant_id = p_tenant_id 
      AND isp.snapshot_date = v_snapshot_date
    GROUP BY isp.fc_id, isp.store_id, m.size
  ),
  ...
  velocity AS (
    ...
    JOIN inv_sku_fc_mapping m ON m.sku = d.sku AND m.tenant_id = d.tenant_id AND m.is_active = true
    JOIN inv_stores s ON s.id = d.store_id AND s.is_transfer_eligible = true AND s.location_type = 'store'
    JOIN _multi_size_fcs_t msf ON msf.fc_id = d.fc_id
    WHERE d.tenant_id = p_tenant_id AND d.avg_daily_sales > 0
    GROUP BY d.fc_id, d.store_id, m.size
  ),
  store_size AS (
  ...
  matches AS (
    ...
    LEFT JOIN _fc_prices_t fp ON fp.fc_id = src.fc_id
  ...
  SELECT
    ...
    f.source_store_id, f.dest_store_id, v_snapshot_date,
    f.transfer_qty,
