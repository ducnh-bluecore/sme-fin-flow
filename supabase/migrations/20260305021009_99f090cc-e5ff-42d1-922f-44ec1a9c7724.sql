CREATE INDEX IF NOT EXISTS idx_inv_lifecycle_batches_tenant_fc_first_sale
ON public.inv_lifecycle_batches (tenant_id, fc_id, first_sale_date);

CREATE INDEX IF NOT EXISTS idx_inv_sku_fc_mapping_tenant_fc_sku
ON public.inv_sku_fc_mapping (tenant_id, fc_id, sku);

CREATE INDEX IF NOT EXISTS idx_cdp_order_items_tenant_sku_order
ON public.cdp_order_items (tenant_id, sku, order_id);

CREATE INDEX IF NOT EXISTS idx_cdp_orders_tenant_id_order_at
ON public.cdp_orders (tenant_id, id, order_at);
...
BEGIN
  ...
  SET first_sale_date = mo.min_order_date
  FROM mapped_orders mo
  WHERE lb.tenant_id = p_tenant_id
    AND lb.fc_id = mo.fc_id
    AND lb.first_sale_date IS NULL;
END;
$$;
  ...
  latest_stock AS (
    ...
    WHERE sp.tenant_id = p_tenant_id
      AND sp.snapshot_date = (
        SELECT MAX(sp2.snapshot_date)
        FROM public.inv_state_positions sp2
