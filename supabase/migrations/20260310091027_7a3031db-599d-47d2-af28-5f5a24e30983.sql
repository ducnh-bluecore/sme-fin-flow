...
BEGIN
  ...
  -- Pre-compute store totals to avoid window function inside aggregate
  CREATE TEMP TABLE _store_totals ON COMMIT DROP AS
  SELECT fc_id, store_id, SUM(on_hand) AS total_on_hand
  FROM _size_detail
  GROUP BY fc_id, store_id;

  -- Step 2: Aggregate into composite score
  ...
  SELECT 
    v_tid, sd.fc_id::text, sd.store_id, p_as_of_date,
    LEAST(100, GREATEST(0, (
      v_w_missing * (COUNT(CASE WHEN sd.on_hand > v_min_stock THEN 1 END)::numeric / NULLIF(COUNT(*), 0))
      + v_w_core * (
          CASE WHEN COUNT(CASE WHEN sd.is_core THEN 1 END) = 0 THEN 1.0
          ELSE COUNT(CASE WHEN sd.is_core AND sd.on_hand > v_min_stock THEN 1 END)::numeric 
               / NULLIF(COUNT(CASE WHEN sd.is_core THEN 1 END), 0)
          END
        )
      + v_w_curve * (
          CASE WHEN SUM(sd.ideal_ratio) = 0 THEN 1.0
          ELSE GREATEST(0, 1.0 - LEAST(1.0, 
            SUM(ABS(
              sd.on_hand::numeric / NULLIF(st.total_on_hand, 0)
              - sd.ideal_ratio
            )) / NULLIF(COUNT(*), 0) / NULLIF(v_max_dev, 0)
          ))
          END
        )
      + v_w_depth * (COUNT(CASE WHEN sd.on_hand > v_shallow THEN 1 END)::numeric / NULLIF(COUNT(*), 0))
    ) * 100)::int),
    CASE 
      WHEN COUNT(CASE WHEN sd.on_hand > v_min_stock THEN 1 END) = COUNT(*) THEN 'full_curve'
      WHEN COUNT(CASE WHEN sd.on_hand > v_min_stock THEN 1 END) = 0 THEN 'empty'
      WHEN COUNT(CASE WHEN sd.on_hand > v_min_stock THEN 1 END)::numeric / COUNT(*) >= 0.7 THEN 'minor_break'
      ELSE 'major_break'
    END,
    LEAST(100, (COUNT(CASE WHEN sd.on_hand <= v_min_stock THEN 1 END)::numeric / NULLIF(COUNT(*), 0) * 100)::int),
    EXISTS(SELECT 1 FROM unnest(v_core_sizes) cs WHERE cs = ANY(ARRAY_AGG(CASE WHEN sd.on_hand <= v_min_stock AND sd.is_core THEN sd.size_code END))),
    COUNT(CASE WHEN sd.on_hand BETWEEN 1 AND v_shallow THEN 1 END)::int
  FROM _size_detail sd
  JOIN _store_totals st ON st.fc_id = sd.fc_id AND st.store_id = sd.store_id
  GROUP BY sd.fc_id, sd.store_id, st.total_on_hand
  HAVING COUNT(CASE WHEN sd.on_hand > 0 THEN 1 END) > 0;
