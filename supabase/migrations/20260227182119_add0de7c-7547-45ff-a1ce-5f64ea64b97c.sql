...
BEGIN
  ...
  SELECT COALESCE((SELECT constraint_value FROM inv_constraint_registry WHERE tenant_id = p_tenant_id AND constraint_key = 'v2_priority_order' AND is_active = true LIMIT 1), '["customer_orders","store_orders","top_fc"]'::jsonb) INTO v_v2_priority_order;
  ...
  CREATE TEMP TABLE tmp_retail_stores ON COMMIT DROP AS
  ...
  CREATE TEMP TABLE tmp_store_totals ON COMMIT DROP AS
  SELECT store_id, SUM(COALESCE(on_hand,0))::integer AS total_on_hand
  FROM inv_state_positions
  WHERE tenant_id = p_tenant_id
  GROUP BY store_id;
  
  CREATE TEMP TABLE tmp_store_fc_count ON COMMIT DROP AS
  ...
  CREATE TEMP TABLE tmp_demand ON COMMIT DROP AS
  SELECT store_id, fc_id, 
  ...
  CREATE TEMP TABLE tmp_size_int ON COMMIT DROP AS
  ...
  CREATE TEMP TABLE tmp_fc_sku_count ON COMMIT DROP AS
  ...
  -- V1 scope: recent collections + new collections + FCs without collection
  CREATE TEMP TABLE tmp_recent_collections ON COMMIT DROP AS
  ...
  CREATE TEMP TABLE tmp_v1_scope ON COMMIT DROP AS
  SELECT DISTINCT f.id AS fc_id
  FROM tmp_fcs f
  LEFT JOIN inv_collections c ON c.id = f.collection_id AND c.tenant_id = p_tenant_id
  WHERE f.collection_id IS NULL
     OR c.is_new_collection = true
     OR f.collection_id IN (SELECT id FROM tmp_recent_collections);
  ...
  IF p_run_type IN ('V1', 'both') THEN
    FOR fc_rec IN 
      ...
      WHERE cw.available > 0
        AND (NOT v_no_broken_size OR COALESCE((SELECT si.is_complete FROM tmp_size_int si WHERE si.fc_id = vs.fc_id LIMIT 1), true))
    LOOP
      v_cw_available := fc_rec.cw_available;
      
      v_fc_sku_count := COALESCE((SELECT sku_count FROM tmp_fc_sku_count WHERE fc_id = fc_rec.fc_id LIMIT 1), 1);
      ...
      FOR store_rec IN SELECT * FROM tmp_retail_stores ORDER BY tier_order LOOP
        ...
        v_total_sku_at_store := COALESCE((SELECT fc_count FROM tmp_store_fc_count WHERE store_id = store_rec.id LIMIT 1), 0);
        SELECT COALESCE(
        ...
        SELECT COALESCE(p.on_hand, 0) + COALESCE(p.in_transit, 0)
        INTO v_current_stock
        FROM tmp_positions p WHERE p.store_id = store_rec.id AND p.fc_id = fc_rec.fc_id;
        ...
        IF store_rec.capacity > 0 THEN
          v_current_total_on_hand := COALESCE((SELECT total_on_hand FROM tmp_store_totals WHERE store_id = store_rec.id LIMIT 1), 0);
        ...
        v_velocity := COALESCE((SELECT velocity FROM tmp_demand WHERE store_id = store_rec.id AND fc_id = fc_rec.fc_id LIMIT 1), 0);
  ...
  IF p_run_type IN ('V2', 'both') THEN
    CREATE TEMP TABLE tmp_v2_entries ON COMMIT DROP AS
    WITH demand_scored AS (
      ...
      JOIN tmp_demand d ON d.store_id = rs.id
      ...
      WHERE (NOT v_no_broken_size OR COALESCE((SELECT si.is_complete FROM tmp_size_int si WHERE si.fc_id = f.id LIMIT 1), true))
    )
    ...
    FOR entry_rec IN SELECT * FROM tmp_v2_entries LOOP
      v_cw_available := COALESCE((SELECT available FROM tmp_cw_stock WHERE fc_id = entry_rec.fc_id LIMIT 1), 0);
      IF v_cw_available <= 0 THEN CONTINUE; END IF;
      
      v_fc_sku_count := COALESCE((SELECT sku_count FROM tmp_fc_sku_count WHERE fc_id = entry_rec.fc_id LIMIT 1), 1);
      ...
      IF entry_rec.capacity > 0 THEN
        v_current_total_on_hand := COALESCE((SELECT total_on_hand FROM tmp_store_totals WHERE store_id = entry_rec.store_id LIMIT 1), 0);
