...
BEGIN
  ...
  -- Calculate new customer trend (avg last 3 months)
  SELECT 
  ...
  FROM (
    SELECT 
      ...
      COUNT(DISTINCT CASE WHEN o.order_at = c.first_order_date THEN o.customer_id END) AS monthly_new,
      AVG(CASE WHEN o.order_at = c.first_order_date THEN o.net_revenue END) AS first_aov
    FROM cdp_orders o
  ...
  FOR v_i IN 0..(p_horizon_months - 1) LOOP
    ...
    LOOP
      ...
      -- Monthly revenue per returning customer (avg_revenue / avg_orders gives per-order, multiply by monthly freq)
