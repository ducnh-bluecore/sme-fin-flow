...
BEGIN
  ...
  -- Aggregate demand to avoid duplicates per (store_id, fc_id)
  CREATE TEMP TABLE tmp_dem ON COMMIT DROP AS 
  SELECT store_id, fc_id, 
    MAX(COALESCE(avg_daily_sales, sales_velocity, 0))::numeric AS vel,
    MAX(COALESCE(customer_orders_qty, 0))::integer AS co,
    MAX(COALESCE(store_orders_qty, 0))::integer AS so
  FROM inv_state_demand 
  WHERE tenant_id=p_tenant_id AND fc_id IN (SELECT fc_id FROM tmp_cw)
