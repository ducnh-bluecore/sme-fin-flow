...
-- Optimize _fc_prices: use inv_sku_fc_mapping instead of expensive LIKE join
DROP FUNCTION IF EXISTS public.compute_inventory_kpi_all(uuid, date);
...
BEGIN
  ...
  -- OPTIMIZED: Use inv_sku_fc_mapping instead of expensive LIKE join
  ...
  WHERE m.tenant_id = p_tenant_id 
    ...
    AND (p.selling_price > 0 OR p.cost_price > 0)
  GROUP BY m.fc_id;
  ...
  SELECT 
    ...
    CASE WHEN st.store_total > 0 THEN
      1.0 - LEAST(1.0, AVG(ABS(d.on_hand::numeric / NULLIF(st.store_total, 0) - d.ideal_ratio)) / v_max_dev)
    ELSE 0 END,
...
-- Also optimize compute_size_transfers
DROP FUNCTION IF EXISTS public.compute_size_transfers(uuid, date);
...
BEGIN
  ...
  -- OPTIMIZED: Use inv_sku_fc_mapping instead of LIKE join
  CREATE TEMP TABLE _fc_prices ON COMMIT DROP AS
