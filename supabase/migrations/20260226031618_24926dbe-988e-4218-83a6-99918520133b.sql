...
BEGIN
  ...
  -- =========================================================
  -- Aggregate inv_state_positions → product-level for KPI tables
  -- =========================================================
  -- Use a CTE to aggregate SKU-level positions to product (fc_id) level
  -- and join with demand data

  -- 1) IDI → state_lost_revenue_daily (closest match)
  ...
  SELECT
    ...
    CASE
      WHEN SUM(sp.on_hand) = 0 THEN 'stockout'
  ...
  ON CONFLICT (tenant_id, product_id, as_of_date)
  DO UPDATE SET
    lost_units_est = EXCLUDED.lost_units_est,
    lost_revenue_est = EXCLUDED.lost_revenue_est,
    driver = EXCLUDED.driver;
  GET DIAGNOSTICS v_idi_count = ROW_COUNT;

  -- 2) Cash Lock → state_cash_lock_daily
  ...
  SELECT
    ...
    CASE WHEN COALESCE(d.velocity_30d, 0) > 0
         THEN LEAST(180, SUM(sp.on_hand) / d.velocity_30d)::int
         ELSE 180 END,
    CASE
      WHEN COALESCE(d.velocity_30d, 0) = 0 THEN 'dead_stock'
  ...
  DO UPDATE SET
    ...
    cash_locked_value = EXCLUDED.cash_locked_value,
    locked_pct = EXCLUDED.locked_pct,
    expected_release_days = EXCLUDED.expected_release_days,
    lock_driver = EXCLUDED.lock_driver;
  GET DIAGNOSTICS v_scs_count = ROW_COUNT;

  -- 3) Margin Leak → state_margin_leak_daily
  ...
  SELECT
    ...
    CASE
      WHEN COALESCE(d.velocity_30d, 0) = 0 THEN 'dead_stock_holding'
  ...
  ON CONFLICT (tenant_id, product_id, as_of_date)
  DO UPDATE SET
    margin_leak_value = EXCLUDED.margin_leak_value,
    leak_driver = EXCLUDED.leak_driver,
    leak_detail = EXCLUDED.leak_detail,
    cumulative_leak_30d = EXCLUDED.cumulative_leak_30d;
  GET DIAGNOSTICS v_chi_count = ROW_COUNT;

  -- 4) Size Health → state_size_health_daily
  ...
  SELECT
    ...
    sp.store_id,
    p_as_of_date,
    -- size_health_score: ratio of sizes with stock vs total sizes
    LEAST(100, GREATEST(0,
      (COUNT(CASE WHEN sp.on_hand > 0 THEN 1 END)::numeric / NULLIF(COUNT(*), 0) * 100)::int
    )),
    CASE
      WHEN COUNT(CASE WHEN sp.on_hand > 0 THEN 1 END) = COUNT(*) THEN 'full_curve'
  ...
  ON CONFLICT (tenant_id, product_id, store_id, as_of_date)
  DO UPDATE SET
    size_health_score = EXCLUDED.size_health_score,
    curve_state = EXCLUDED.curve_state,
    deviation_score = EXCLUDED.deviation_score,
    core_size_missing = EXCLUDED.core_size_missing,
    shallow_depth_count = EXCLUDED.shallow_depth_count;
  GET DIAGNOSTICS v_size_count = ROW_COUNT;

  -- 5) Markdown Risk → state_markdown_risk_daily
  ...
  SELECT
    ...
    CASE WHEN COALESCE(d.velocity_30d, 0) > 0
         THEN GREATEST(0, (SUM(sp.on_hand) / d.velocity_30d - 60))::int
         ELSE 0 END,
    CASE
      WHEN COALESCE(d.velocity_30d, 0) = 0 THEN 'dead_stock'
      WHEN SUM(sp.on_hand) / NULLIF(d.velocity_30d, 0) > 90 THEN 'severe_overstock'
  ...
  DO UPDATE SET
    ...
    markdown_eta_days = EXCLUDED.markdown_eta_days,
    reason = EXCLUDED.reason;
  GET DIAGNOSTICS v_md_count = ROW_COUNT;

  -- 6) Evidence Packs → si_evidence_packs
  ...
  SELECT
    ...
    CASE
      ...
      ELSE 'medium'
    END,
    'FC ' || sp.fc_id || ': ' || SUM(sp.on_hand) || ' units, DOC=' ||
      CASE WHEN COALESCE(d.velocity_30d, 0) > 0
           THEN ROUND(SUM(sp.on_hand) / d.velocity_30d)::text
           ELSE '∞' END,
    jsonb_build_object(
      'on_hand', SUM(sp.on_hand),
      'velocity_30d', COALESCE(d.velocity_30d, 0),
      'doc', CASE WHEN COALESCE(d.velocity_30d, 0) > 0 THEN ROUND(SUM(sp.on_hand)/d.velocity_30d) ELSE 180 END,
  ...
  WHERE sp.tenant_id = v_tid
    ...
    AND sp.on_hand > 0
  GROUP BY sp.fc_id, d.velocity_30d
  HAVING COALESCE(d.velocity_30d, 0) = 0
     OR SUM(sp.on_hand) / NULLIF(d.velocity_30d, 0) > 60
  ON CONFLICT (tenant_id, product_id, as_of_date, evidence_type)
  DO UPDATE SET
