...
BEGIN
  ...
  -- Precompute product-level aggregates into a temp table
  ...
  LEFT JOIN (
    SELECT id, selling_price, cost_price FROM products WHERE tenant_id = p_tenant_id
  ) p ON p.id = sp.fc_id
  ...
  SELECT v_tid, fc_id, p_as_of_date,
  ...
  SELECT v_tid, sp.fc_id, sp.store_id, p_as_of_date,
  ...
  SELECT v_tid, fc_id, p_as_of_date, 'overstock_alert',
