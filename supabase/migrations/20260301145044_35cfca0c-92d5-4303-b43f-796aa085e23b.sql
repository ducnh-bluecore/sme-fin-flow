...
AS $function$
  ...
  latest_store_snapshot AS (
    SELECT p.store_id, MAX(p.snapshot_date) AS snapshot_date
    FROM inv_state_positions p
    WHERE p.tenant_id = p_tenant_id::uuid
    GROUP BY p.store_id
  ),
  sku_fc_map AS (
    SELECT DISTINCT m.sku, m.fc_id
  ...
  product_stock AS (
    SELECT
      sfm.fc_id::text AS product_id,
      COALESCE(SUM(COALESCE(p.on_hand, 0)), 0)::bigint AS total_on_hand
    FROM latest_store_snapshot ls
    JOIN inv_state_positions p
      ON p.store_id = ls.store_id
     AND p.snapshot_date = ls.snapshot_date
     AND p.tenant_id = p_tenant_id::uuid
    JOIN sku_fc_map sfm
      ON sfm.sku = p.sku
    GROUP BY sfm.fc_id
  ),
