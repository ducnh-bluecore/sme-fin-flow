...
DECLARE
  ...
  v_cohort_json jsonb;
  v_seasonal_multipliers jsonb; -- YoY seasonal multipliers
...
BEGIN
  ...
  -- === YoY SEASONAL MULTIPLIERS ===
  -- Compare each target month with same month last year vs trailing avg
  -- This captures spikes like 11.11, Black Friday, Tet, etc.
  ...
  FOR seasonal_rec IN
    ...
    -- For each month, compute: ratio of that month vs avg of surrounding 2 months
    seasonal_index AS (
      SELECT 
        EXTRACT(MONTH FROM m)::int as month_num,
        EXTRACT(YEAR FROM m)::int as yr,
        rev,
        -- Average of previous and next month as baseline
        (LAG(rev) OVER (ORDER BY m) + LEAD(rev) OVER (ORDER BY m)) / 2.0 as surround_avg
      FROM monthly_rev
    ),
    -- Average seasonal index per calendar month (across available years)
    avg_seasonal AS (
      SELECT 
        month_num,
        AVG(CASE WHEN surround_avg > 0 THEN rev / surround_avg ELSE 1.0 END) as seasonal_idx,
    ...
    SELECT month_num, 
      CASE 
        WHEN sample_count >= 2 THEN GREATEST(0.7, LEAST(1.8, seasonal_idx)) -- cap between 0.7x and 1.8x
        WHEN sample_count = 1 THEN GREATEST(0.8, LEAST(1.5, seasonal_idx)) -- less trust with 1 sample
        ELSE 1.0 
      END as multiplier,
      sample_count
    FROM avg_seasonal
  ...
  FOR i IN 0..(p_horizon_months - 1) LOOP
    ...
    -- Get seasonal multiplier for this target month
    ...
    IF i = 0 THEN
      v_returning_revenue := v_returning_base * v_growth_factor * v_seasonal_mult;
    ELSE
      v_returning_revenue := v_returning_base * POWER(v_monthly_decay, i) * v_growth_factor * v_seasonal_mult;
    END IF;
    
    v_new_revenue := v_new_base * v_growth_factor * v_seasonal_mult;
    v_anon_revenue := v_anon_base * v_growth_factor * v_seasonal_mult;
    ...
    DECLARE
      ...
      j int;
    BEGIN
      v_scale := CASE WHEN i = 0 THEN v_seasonal_mult ELSE POWER(v_monthly_decay, i) * v_growth_factor * v_seasonal_mult END;
      ...
      v_result := v_result || jsonb_build_object(
        ...
        'seasonal_multiplier', ROUND(v_seasonal_mult, 3)
      );
    END;
  END LOOP;
