...
BEGIN
  ...
  FROM inv_family_codes f WHERE f.tenant_id=p_tenant_id AND f.is_active=true AND f.id IN (SELECT fc_id FROM tmp_cw)
    ...
    AND f.fc_code NOT LIKE 'LB%'
    AND f.fc_code NOT LIKE 'GC%'
    AND f.fc_code NOT LIKE 'VC0%'
    AND f.fc_code NOT LIKE 'RFID%'
    AND f.fc_code NOT LIKE 'DTR%'
    AND f.fc_code NOT LIKE 'SER%'
    AND f.fc_code NOT LIKE 'TXN%'
    AND f.fc_code NOT LIKE '333%'
    AND f.fc_code NOT LIKE 'CLI%'
    AND f.fc_code NOT LIKE 'OBG%'
    AND f.fc_code NOT LIKE 'BVSE%';
  CREATE INDEX ON tmp_fc(id);

  -- ── Position data: latest snapshot per store ──
  ...
  FROM inv_state_positions sp
  JOIN tmp_latest_snap ls ON ls.store_id = sp.store_id AND sp.snapshot_date = ls.max_date
  WHERE sp.tenant_id=p_tenant_id GROUP BY store_id;
