...
-- 1. FDP TRUTH SNAPSHOT (Financial Data Platform)
...
-- 2. MDP TRUTH SNAPSHOT (Marketing Data Platform)
...
-- 3. CDP TRUTH SNAPSHOT (Customer Data Platform)
-- Using v_cdp_equity_overview which has all needed fields
...
equity_data AS (
  SELECT 
    tenant_id,
    total_customers as equity_total_customers,
    customers_with_equity,
    total_equity_12m,
    total_equity_24m,
...
SELECT 
  COALESCE(cc.tenant_id, ed.tenant_id) as tenant_id,
  
  -- Customer Counts
  COALESCE(cc.total_customers, ed.equity_total_customers, 0) as total_customers,
  COALESCE(cc.active_customers, 0) as active_customers,
  -- at_risk derived from equity view's at_risk_percent
  CASE WHEN ed.at_risk_percent > 0 AND ed.equity_total_customers > 0
    THEN ROUND((ed.at_risk_percent / 100.0) * ed.equity_total_customers)
    ELSE 0
  ...
  -- Ratios
  COALESCE(ed.at_risk_percent, 0) as at_risk_pct,
  
  -- LTV
  COALESCE(ed.avg_ltv, 0) as avg_ltv,
  
  -- LTV/CAC (join với central_metrics)
  COALESCE(
    (SELECT ltv_cac_ratio FROM central_metrics_snapshots cms 
     WHERE cms.tenant_id = COALESCE(cc.tenant_id, ed.tenant_id)
     ORDER BY snapshot_at DESC LIMIT 1), 
    0
  ) as ltv_cac_ratio,
  
  COALESCE(ed.last_updated, now()) as snapshot_at

FROM customer_counts cc
FULL OUTER JOIN equity_data ed ON cc.tenant_id = ed.tenant_id;
