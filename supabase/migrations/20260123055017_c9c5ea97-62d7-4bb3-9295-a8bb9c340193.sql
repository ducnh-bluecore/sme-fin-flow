...
-- Fix: Use MAX(order_at) as reference date instead of CURRENT_DATE
-- This ensures detection works with historical data
...
BEGIN
  ...
  v_current_end := v_reference_date + 1; -- Include the last day
  ...
  SELECT 
    ...
    (t.current_margin_pct IS NOT NULL AND t.baseline_margin_pct IS NOT NULL 
     AND ABS(t.current_margin_pct - t.baseline_margin_pct) > 3),
    ...
    ROUND(ABS(t.total_baseline_margin - t.total_current_margin), 0),
    t.total_customers,
    ...
    ' ' || cdp_format_currency(ABS(t.total_baseline_margin - t.total_current_margin)) || ' lợi nhuận. ' ||
    '(Dữ liệu: ' || to_char(v_current_start, 'DD/MM/YYYY') || ' - ' || to_char(v_reference_date, 'DD/MM/YYYY') || ')'
