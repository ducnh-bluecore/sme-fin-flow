...
DECLARE
  ...
  v_tid TEXT := p_tenant_id::text; -- cast once for text-typed tables
...
BEGIN
  ...
  -- SCS (tenant_id is UUID, store_id is UUID)
  ...
  -- Size Health (network) - uses TEXT tenant_id via v_tid
  DELETE FROM state_size_health_daily WHERE tenant_id=v_tid AND as_of_date=p_as_of_date AND store_id IS NULL;
  ...
  SELECT v_tid,fc_id,NULL,p_as_of_date,
  ...
  SELECT v_tid,fm.fc_id,p_as_of_date,
  ...
  -- Markdown Risk - uses v_tid
  DELETE FROM state_markdown_risk_daily WHERE tenant_id=v_tid AND as_of_date=p_as_of_date;
  WITH h AS (SELECT product_id,size_health_score,curve_state FROM state_size_health_daily WHERE tenant_id=v_tid AND as_of_date=p_as_of_date AND store_id IS NULL),
  ...
  SELECT v_tid,product_id,p_as_of_date,rr,
  ...
  -- Per-store Health - uses v_tid
  DELETE FROM state_size_health_daily WHERE tenant_id=v_tid AND as_of_date=p_as_of_date AND store_id IS NOT NULL;
  ...
  INSERT INTO state_size_health_daily(tenant_id,product_id,store_id,as_of_date,size_health_score,curve_state,deviation_score,core_size_missing,shallow_depth_count)
  SELECT v_tid,fc_id,store_id,p_as_of_date,
    GREATEST(0,LEAST(100,100-LEAST(40,ds*20)-CASE WHEN cm THEN 30 ELSE 0 END-sc*5)),
    CASE WHEN GREATEST(0,LEAST(100,100-LEAST(40,ds*20)-CASE WHEN cm THEN 30 ELSE 0 END-sc*5))<40 THEN 'broken'
         WHEN GREATEST(0,LEAST(100,100-LEAST(40,ds*20)-CASE WHEN cm THEN 30 ELSE 0 END-sc*5))<60 THEN 'risk'
         WHEN GREATEST(0,LEAST(100,100-LEAST(40,ds*20)-CASE WHEN cm THEN 30 ELSE 0 END-sc*5))<80 THEN 'watch' ELSE 'healthy' END,
  ...
  SELECT v_tid,sh.product_id,p_as_of_date,
  ...
  WHERE sh.tenant_id=v_tid AND sh.as_of_date=p_as_of_date AND sh.store_id IS NULL AND sh.curve_state IN('broken','risk','watch');
  GET DIAGNOSTICS v_cash_lock_count = ROW_COUNT;

  -- Margin Leak - uses v_tid
  ...
  SELECT v_tid,product_id,p_as_of_date,ROUND(lost_revenue_est*0.4),'size_break',
    jsonb_build_object('lost_units',lost_units_est,'lost_revenue',lost_revenue_est,'driver',driver),ROUND(lost_revenue_est*0.4)
  FROM state_lost_revenue_daily WHERE tenant_id=v_tid AND as_of_date=p_as_of_date AND lost_revenue_est>0;
  INSERT INTO state_margin_leak_daily(tenant_id,product_id,as_of_date,margin_leak_value,leak_driver,leak_detail,cumulative_leak_30d)
  SELECT v_tid,md.product_id,p_as_of_date,
  ...
  WHERE md.tenant_id=v_tid AND md.as_of_date=p_as_of_date AND md.markdown_risk_score>=60 AND COALESCE(fs.total_stock,0)>0;
  GET DIAGNOSTICS v_margin_leak_count = ROW_COUNT;

  -- Evidence Packs - uses v_tid (si_evidence_packs.tenant_id is UUID)
  ...
  LEFT JOIN state_lost_revenue_daily lr ON lr.tenant_id=v_tid AND lr.as_of_date=p_as_of_date AND lr.product_id=sh.product_id
  LEFT JOIN state_markdown_risk_daily md ON md.tenant_id=v_tid AND md.as_of_date=p_as_of_date AND md.product_id=sh.product_id
  LEFT JOIN state_cash_lock_daily cl ON cl.tenant_id=v_tid AND cl.as_of_date=p_as_of_date AND cl.product_id=sh.product_id
  WHERE sh.tenant_id=v_tid AND sh.as_of_date=p_as_of_date AND sh.store_id IS NULL AND sh.curve_state IN('broken','risk');
