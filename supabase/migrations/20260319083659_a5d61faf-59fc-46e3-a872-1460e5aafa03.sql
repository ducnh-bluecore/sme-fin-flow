...
DECLARE
  ...
  v_overall_yoy numeric;         -- NEW: overall YoY for LY-anchor
  v_ly_monthly_rev jsonb;         -- NEW: same-month-LY revenue lookup
  i int;
  cohort_rec record;
  seasonal_rec record;
  yoy_rec record;
...
BEGIN
  ...
  -- === OVERALL YoY GROWTH (for LY-anchor method) ===
  SELECT COALESCE(
    SUM(CASE WHEN EXTRACT(YEAR FROM m) = EXTRACT(YEAR FROM v_latest_order_month) THEN rev END) /
    NULLIF(SUM(CASE WHEN EXTRACT(YEAR FROM m) = EXTRACT(YEAR FROM v_latest_order_month) - 1 THEN rev END), 0),
    1.0
  ) INTO v_overall_yoy
  FROM (
  ...
  WHERE EXTRACT(MONTH FROM m) <= EXTRACT(MONTH FROM v_latest_order_month);
  -- Cap overall YoY
  v_overall_yoy := GREATEST(0.7, LEAST(2.0, v_overall_yoy));

  -- === LY MONTHLY REVENUE LOOKUP ===
  v_ly_monthly_rev := '{}'::jsonb;
  FOR ly_rec IN
    SELECT EXTRACT(MONTH FROM date_trunc('month', order_at))::int as month_num,
           SUM(net_revenue)::numeric as rev
    FROM cdp_orders
    WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
      AND order_at >= (v_latest_order_month - interval '24 months')
      AND order_at < (v_latest_order_month - interval '11 months')
      AND EXTRACT(YEAR FROM order_at) = EXTRACT(YEAR FROM v_latest_order_month) - 1
    GROUP BY 1
  LOOP
    v_ly_monthly_rev := v_ly_monthly_rev || jsonb_build_object(ly_rec.month_num::text, ly_rec.rev);
  END LOOP;

  -- === YoY GROWTH FACTORS — smoothed with 50% direct + 50% rolling ===
  v_yoy_growth_factors := '{}'::jsonb;
  FOR yoy_rec IN
    WITH monthly_rev AS (
      SELECT 
    ...
    rolling_avg AS (
      SELECT m, month_num, yr, rev,
        AVG(rev) OVER (PARTITION BY yr ORDER BY m ROWS BETWEEN 2 PRECEDING AND 3 FOLLOWING) as rolling6
      FROM monthly_rev
    ),
    yoy_pairs AS (
      SELECT 
        a.month_num,
        CASE WHEN b.rev > 0 AND b.rolling6 > 0 THEN
          0.5 * (a.rev / b.rev) + 0.5 * (a.rolling6 / b.rolling6)
        WHEN b.rev > 0 THEN a.rev / b.rev
        ELSE NULL END as blended_yoy
      FROM rolling_avg a
      JOIN rolling_avg b ON a.month_num = b.month_num AND a.yr = b.yr + 1
    )
    SELECT month_num, AVG(blended_yoy) as avg_yoy
    FROM yoy_pairs WHERE blended_yoy IS NOT NULL
    GROUP BY month_num
  LOOP
    v_yoy_growth_factors := v_yoy_growth_factors || 
      jsonb_build_object(yoy_rec.month_num::text, GREATEST(0.7, LEAST(2.0, yoy_rec.avg_yoy)));
  END LOOP;

  -- === SEASONAL MULTIPLIERS with dynamic cap ===
  v_seasonal_multipliers := '{}'::jsonb;
  FOR seasonal_rec IN
    WITH monthly_rev AS (
      SELECT date_trunc('month', order_at)::date as m, SUM(net_revenue) as rev
    ...
    with_stats AS (
      SELECT m, EXTRACT(MONTH FROM m)::int as month_num, rev,
        (LAG(rev) OVER (ORDER BY m) + LEAD(rev) OVER (ORDER BY m)) / 2.0 as surround_avg,
        STDDEV(rev) OVER () as global_stddev, AVG(rev) OVER () as global_avg
      FROM monthly_rev
    ),
    seasonal_index AS (
      SELECT month_num,
  ...
  -- === BASE CALCULATION: 60% median + 40% weighted avg ===
  WITH monthly_totals AS (
    SELECT date_trunc('month', order_at)::date as m,
  ...
  new_counts AS (
    ...
    WHERE o.tenant_id = p_tenant_id AND o.status NOT IN ('cancelled','returned')
      ...
      AND NOT EXISTS (
        SELECT 1 FROM cdp_orders o2 WHERE o2.tenant_id = p_tenant_id AND o2.status NOT IN ('cancelled','returned')
  ...
  FOR cohort_rec IN
    ...
    cohort_summary AS (
      SELECT to_char(date_trunc('month', first_at), 'YYYY-MM') as cohort_month,
        COUNT(*) as cohort_active, SUM(latest_rev) as cohort_rev
      FROM recent_customers WHERE date_trunc('month', first_at) < v_latest_order_month
      GROUP BY 1
    )
    SELECT cohort_month, cohort_active, cohort_rev
    FROM cohort_summary WHERE cohort_rev > 0 ORDER BY cohort_rev DESC LIMIT 12
  LOOP
    v_cohort_json := v_cohort_json || jsonb_build_object(
      'cohort_month', cohort_rec.cohort_month, 'cohort_size', cohort_rec.cohort_active,
      'retention_pct', 0, 'returning_customers', cohort_rec.cohort_active,
      'revenue', ROUND(cohort_rec.cohort_rev)
    );
  END LOOP;

  -- === GENERATE FORECAST with SMART ENSEMBLE ===
  FOR i IN 0..(p_horizon_months - 1) LOOP
    ...
    DECLARE
      ...
      v_trend_total numeric;
      v_ly_rev numeric;
      v_ly_forecast numeric;
      v_final_total numeric;
      v_blend_ratio numeric;  -- how much to use trend (vs LY)
    BEGIN
      -- TREND METHOD: seasonal * sqrt(yoy), capped 0.6-1.5
      v_combined_mult := v_seasonal_mult * POWER(v_yoy_factor, 0.5);
      v_combined_mult := GREATEST(0.6, LEAST(1.5, v_combined_mult));
      ...
      -- LY-ANCHOR METHOD: same_month_LY * overall_YoY
      ...
      -- SMART BLEND: if trend and LY diverge > 30%, pull toward LY (conservative)
      -- This catches post-spike corrections where trend overshoots
      IF v_ly_forecast > 0 AND ABS(v_trend_total - v_ly_forecast) / v_ly_forecast > 0.30 THEN
        v_blend_ratio := 0.4;  -- 40% trend, 60% LY when they diverge
      ELSE
        v_blend_ratio := 0.7;  -- 70% trend, 30% LY when they agree
      END IF;
      
      -- Only apply ensemble if we have LY data
