...
BEGIN
  ...
  SELECT sp.fc_id, SUM(sp.on_hand) AS total_on_hand,
    ...
    COALESCE(p.selling_price, 0) AS price, COALESCE(p.cost_price, 0) AS cost
  ...
  LEFT JOIN (SELECT id, selling_price, cost_price FROM products WHERE tenant_id = p_tenant_id) p ON p.id = sp.fc_id
  ...
  SELECT v_tid, sp.fc_id::text, sp.store_id, p_as_of_date,
    ...
    COUNT(CASE WHEN sp.on_hand=0 THEN 1 END) > 0,  -- boolean: core_size_missing
