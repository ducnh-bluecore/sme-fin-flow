CREATE OR REPLACE FUNCTION public.fn_allocation_engine(p_tenant_id uuid, p_run_id uuid, p_run_type text DEFAULT 'both'::text, p_collection_ids uuid[] DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- ── Load SEM allocation policies ──
  SELECT weights INTO v_base_weights FROM sem_allocation_policies
    WHERE tenant_id=p_tenant_id AND policy_type='BASE' AND is_active=true LIMIT 1;
  SELECT weights INTO v_scarcity_weights FROM sem_allocation_policies
    WHERE tenant_id=p_tenant_id AND policy_type='SCARCITY' AND is_active=true LIMIT 1;
  v_scarcity_active := v_scarcity_weights IS NOT NULL;
  SELECT weights INTO v_dynamic_weights FROM sem_allocation_policies
    WHERE tenant_id=p_tenant_id AND policy_type='DYNAMIC' AND is_active=true LIMIT 1;
  ...
  -- ── Filter non-fashion items + enrich with criticality + COLLECTION FILTER ──
  ...
  WHERE f.tenant_id=p_tenant_id AND f.is_active=true AND f.id IN (SELECT fc_id FROM tmp_cw)
    ...
    -- ★ COLLECTION FILTER: if p_collection_ids provided, only include FCs belonging to those collections
  ...
  -- ── V1: min store stock with BASE policy tier weights + SCARCITY filter ──
  IF p_run_type IN ('V1','both') THEN
    WITH v1_raw AS (
      SELECT f.id AS fid,f.fc_name,f.fc_code,f.is_core_hero,f.criticality_class,f.sku_count,
        s.id AS sid,s.store_name,s.store_code,s.tier,s.capacity,s.tord,
        cw.available AS cwt,
        GREATEST(v_cw_reserve_min,
          CASE f.criticality_class WHEN 'CORE' THEN v_core_hero_min_per_sku*f.sku_count
            WHEN 'HERO' THEN GREATEST((v_core_hero_min_per_sku*f.sku_count*0.7)::integer, v_cw_reserve_min)
            ELSE 0 END
        ) AS cwr,
      ...
      WHERE cw.available>GREATEST(v_cw_reserve_min,CASE WHEN f.is_core_hero THEN v_core_hero_min_per_sku*f.sku_count ELSE 0 END)
        AND (p_collection_ids IS NOT NULL OR f.collection_id IS NULL OR c.is_new_collection=true OR f.collection_id IN (SELECT cc.id FROM inv_collections cc WHERE cc.tenant_id=p_tenant_id AND cc.air_date IS NOT NULL ORDER BY cc.air_date DESC LIMIT v_bst_scope_recent_count))
        AND (NOT v_scarcity_active OR COALESCE(sfs.system_total,0) >= 40 OR COALESCE((v_scarcity_weights->>('tier_'||s.tier))::numeric,0) > 0)
    ),v1_mq AS (
      SELECT *,GREATEST(1, ROUND(COALESCE((SELECT (r->>'min_qty')::integer FROM jsonb_array_elements(v_v1_min_stock_ranges) r WHERE (r->>'max_sku')::integer>=sfc ORDER BY (r->>'max_sku')::integer LIMIT 1),5) * tier_weight))::integer AS mq FROM v1_raw
    ),v1_sh AS (
    ...
    SELECT p_tenant_id,p_run_id,fid,COALESCE(fc_name,fc_code,''),sid,COALESCE(store_name,store_code,''),NULL,aq::integer,cs,
      ...
      format('V1: %s thiếu %s (min=%s,tier=%s,class=%s)',store_name,shortage,mq,tier,criticality_class),aq*v_avg_unit_price,'pending','V1',
      jsonb_build_object('rule','v1_min_store_stock','tier',tier,'min_qty_required',mq,'current_stock',cs,'source_on_hand',cwt,'dest_on_hand',cs,'sold_7d',ROUND(vel*7)::integer,'criticality_class',criticality_class,'tier_weight',tier_weight,'policy','BASE'),
      format('V1: FC %s (%s), CH %s (tier %s, w=%s) min %s, có %s, bổ sung %s',COALESCE(fc_name,fc_code),criticality_class,store_name,tier,ROUND(tier_weight::numeric,1)::text,mq,cs,aq),NULL
    FROM v1_fin WHERE aq>0;
    GET DIAGNOSTICS v_v1_count=ROW_COUNT;
  END IF;

  -- ── V2: demand-based with DYNAMIC policy weights ──
  IF p_run_type IN ('V2','both') THEN
    ...
    WITH v2_raw AS (
      SELECT f.id AS fid,f.fc_name,f.fc_code,f.is_core_hero,f.criticality_class,f.sku_count,s.id AS sid,s.store_name,s.store_code,s.tier,s.capacity,
        cw.available AS cwt,
        GREATEST(v_cw_reserve_min, CASE f.criticality_class WHEN 'CORE' THEN v_core_hero_min_per_sku*f.sku_count WHEN 'HERO' THEN GREATEST((v_core_hero_min_per_sku*f.sku_count*0.7)::integer,v_cw_reserve_min) ELSE 0 END) AS cwr,
        ...
        COALESCE((v_dynamic_weights->>'sales_velocity')::numeric, 0.6) AS w_vel,
        COALESCE((v_dynamic_weights->>'stock_cover')::numeric, 0.3) AS w_cover
      ...
      SELECT *,LEAST(dq,cwt-cwr,CASE WHEN capacity>0 THEN GREATEST(capacity-stoh,0) ELSE dq END) AS abd,
        SUM(LEAST(dq,cwt-cwr,CASE WHEN capacity>0 THEN GREATEST(capacity-stoh,0) ELSE dq END)) OVER (PARTITION BY fid ORDER BY
          CASE pt WHEN 'customer_orders' THEN 0 WHEN 'store_orders' THEN 1 ELSE 2 END,
          (COALESCE(vel,0)*w_vel + (1.0/GREATEST(wc,0.01))*w_cover) DESC
    ...
    SELECT p_tenant_id,p_run_id,fid,COALESCE(fc_name,fc_code,''),sid,COALESCE(store_name,store_code,''),NULL,aq::integer,cs,wc,
      ...
      format('V2: %s — %s cần %s, chia %s (%s)',CASE pt WHEN 'customer_orders' THEN 'ĐH khách' WHEN 'store_orders' THEN 'ĐH CH' ELSE 'Top FC' END,store_name,dq,aq,criticality_class),
      aq*v_avg_unit_price,'pending','V2',
      jsonb_build_object('rule','v2_demand','priority_type',pt,'demand_qty',dq,'source_on_hand',cwt,'dest_on_hand',cs,'sold_7d',ROUND(vel*7)::integer,'criticality_class',criticality_class,'policy','DYNAMIC','w_velocity',w_vel,'w_cover',w_cover),
      format('V2 %s: FC %s (%s), CH %s cần %s, chia %s',CASE pt WHEN 'customer_orders' THEN 'ĐH khách' WHEN 'store_orders' THEN 'ĐH CH' ELSE 'Top FC' END,COALESCE(fc_name,fc_code),criticality_class,store_name,dq,aq),NULL
