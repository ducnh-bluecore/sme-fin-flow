...
-- Function to link cdp_orders.customer_id to cdp_customers
-- Strategy: Phone match first (exact), then Exact Name match (for orders without phone)
...
BEGIN
  ...
  -- Normalize phone: strip spaces, match exactly
  UPDATE cdp_orders o
  ...
  -- PASS 2: Exact name match (only for customers with unique names)
  -- Avoid ambiguous names by only matching when exactly 1 customer has that name
  WITH unique_names AS (
    SELECT name, MIN(id) as customer_id
    FROM cdp_customers
  ...
  -- Count ambiguous names (for reporting)
  SELECT COUNT(DISTINCT name) INTO v_ambiguous_names
  FROM cdp_customers
  WHERE tenant_id = p_tenant_id
    AND name IS NOT NULL
    AND trim(name) != ''
