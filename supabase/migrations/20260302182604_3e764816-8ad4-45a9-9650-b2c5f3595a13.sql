...
-- Drop and recreate fn_batch_size_split_v2 with SEM size curve integration
DROP FUNCTION IF EXISTS public.fn_batch_size_split_v2(uuid, uuid, text, integer);

CREATE OR REPLACE FUNCTION public.fn_batch_size_split_v2(
  p_tenant_id uuid,
  p_run_id uuid,
  p_table_name text,
  p_max_records integer DEFAULT 2000
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_size_ratios jsonb;
BEGIN
  -- Step 0: Load size curve profiles from SEM
  ...
  -- Step 1: Pre-compute CW stock and dest stock
  ...
  LEFT JOIN inv_state_positions sp ON sp.sku = m.sku AND sp.tenant_id = p_tenant_id
  ...
  IF p_table_name = 'alloc' THEN
    ...
    LOOP
      v_total := v_total + 1;
      BEGIN
        -- Match FC category to size curve profile
        v_size_ratios := NULL;
        ...
        LOOP
          -- Use SEM size curve ratio if available, otherwise proportional by CW stock
