...
-- Fix: Increase LIMIT to 500 and ensure broken_system items are included
-- by using a UNION approach: top 200 by risk + all broken-curve items
...
BEGIN
  ...
  base AS (
    SELECT
      ...
      COALESCE(hl.curve_state, 'unknown') = 'broken' AS is_broken_size
    FROM risk
  ...
  -- Top 200 by risk score + ensure ALL broken_system items are included
  ranked AS (
  ...
  final AS (
    ...
    UNION
    SELECT * FROM ranked WHERE cgroup = 'broken_system' AND rn > 200
  )
  SELECT
