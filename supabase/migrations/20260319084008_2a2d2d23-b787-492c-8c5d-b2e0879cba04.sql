...
-- Update divergence threshold from 30% to 50%, and blend from 40/60 to 30/70
-- This is applied by replacing the function with the corrected thresholds
-- The function was already deployed in previous migration, just need to update the two constants
...
AS $$
-- Patching only the blend constants: threshold 0.30->0.50, diverge blend 0.4/0.6->0.30/0.70
-- Full function body required for CREATE OR REPLACE
DECLARE
...
BEGIN
  ...
  SELECT COALESCE(
    ...
    NULLIF(SUM(CASE WHEN EXTRACT(YEAR FROM m) = EXTRACT(YEAR FROM v_latest_order_month) - 1 THEN rev END), 0), 1.0
  ) INTO v_overall_yoy
  FROM (SELECT date_trunc('month', order_at)::date as m, SUM(net_revenue) as rev
    FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
      AND order_at >= (v_latest_order_month - interval '24 months') AND order_at < (v_latest_order_month + interval '1 month')
    GROUP BY 1) sub
  WHERE EXTRACT(MONTH FROM m) <= EXTRACT(MONTH FROM v_latest_order_month);
  v_overall_yoy := GREATEST(0.7, LEAST(2.0, v_overall_yoy));

  -- LY monthly rev lookup
  v_ly_monthly_rev := '{}'::jsonb;
  FOR ly_rec IN
    SELECT EXTRACT(MONTH FROM date_trunc('month', order_at))::int as month_num, SUM(net_revenue)::numeric as rev
    FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
      AND order_at >= (v_latest_order_month - interval '24 months') AND order_at < (v_latest_order_month - interval '11 months')
  ...
  -- YoY growth factors (smoothed)
  v_yoy_growth_factors := '{}'::jsonb;
  FOR yoy_rec IN
    WITH monthly_rev AS (
      SELECT date_trunc('month', order_at)::date as m, EXTRACT(MONTH FROM order_at)::int as month_num,
        EXTRACT(YEAR FROM order_at)::int as yr, SUM(net_revenue) as rev
      FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
        AND order_at >= (v_latest_order_month - interval '24 months') AND order_at < (v_latest_order_month + interval '1 month')
      GROUP BY 1, 2, 3
    ),
  ...
  ELSE
    ...
    INTO v_roas FROM ad_spend_daily WHERE tenant_id = p_tenant_id AND spend_date >= (v_latest_order_month - interval '3 months')::date;
  END IF;

  -- Base: median + weighted avg
  WITH monthly_totals AS (
    SELECT date_trunc('month', order_at)::date as m,
      SUM(CASE WHEN customer_name IS NULL AND (customer_phone IS NULL OR LENGTH(customer_phone)<10) AND customer_id IS NULL THEN net_revenue ELSE 0 END) as anon_rev,
      SUM(CASE WHEN customer_name IS NOT NULL OR (customer_phone IS NOT NULL AND LENGTH(customer_phone)>=10) OR customer_id IS NOT NULL THEN net_revenue ELSE 0 END) as identified_rev
    FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
  ...
  SELECT MAX(CASE WHEN mt.m = v_latest_order_month THEN mt.identified_rev END),
  ...
  FROM (SELECT date_trunc('month', order_at)::date as m,
    SUM(CASE WHEN customer_name IS NOT NULL OR (customer_phone IS NOT NULL AND LENGTH(customer_phone)>=10) OR customer_id IS NOT NULL THEN net_revenue ELSE 0 END) as identified_rev,
    SUM(CASE WHEN customer_name IS NULL AND (customer_phone IS NULL OR LENGTH(customer_phone)<10) AND customer_id IS NULL THEN net_revenue ELSE 0 END) as anon_rev
    FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
  ...
  FROM cdp_orders WHERE tenant_id = p_tenant_id AND status NOT IN ('cancelled','returned')
    AND customer_name IS NOT NULL AND order_at >= v_latest_order_month AND order_at < (v_latest_order_month + interval '1 month') LIMIT 1000;
  ...
  -- FORECAST with SMART ENSEMBLE (threshold=0.50, diverge=30/70, agree=70/30)
  FOR i IN 0..(p_horizon_months - 1) LOOP
    ...
    BEGIN
      ...
      -- FIXED: threshold 50%, diverge blend 30/70
