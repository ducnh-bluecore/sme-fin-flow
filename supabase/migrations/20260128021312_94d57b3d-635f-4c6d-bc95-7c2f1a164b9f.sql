...
-- Fix: Use proper text comparison for product_id matching
-- cdp_order_items.product_id is TEXT, products.id is UUID
-- JOIN should compare as TEXT to avoid UUID cast failures
...
BEGIN
  ...
  LEFT JOIN products p ON p.tenant_id = coi.tenant_id
    AND p.id::text = coi.product_id  -- Compare as TEXT (both directions work)
