...
-- Add filter to exclude single-size FCs from transfer suggestions
...
BEGIN
  ...
  -- NEW: Identify multi-size FCs only (exclude single-SKU / Free Size products)
  ...
  SELECT
    ...
    f.logistics_cost * f.transfer_qty,
    (f.transfer_qty * f.price * 0.8) - (f.logistics_cost * f.transfer_qty),
    f.reason,
    'pending'
  FROM final f
  WHERE (f.transfer_qty * f.price * 0.8) > (f.logistics_cost * f.transfer_qty);

  GET DIAGNOSTICS v_count = ROW_COUNT;

  RETURN jsonb_build_object(
    'success', true,
    'transfer_rows', v_count,
    'snapshot_date', v_snapshot_date,
    'elapsed_ms', EXTRACT(MILLISECOND FROM clock_timestamp() - v_start)::int
  );
END;
$$;
