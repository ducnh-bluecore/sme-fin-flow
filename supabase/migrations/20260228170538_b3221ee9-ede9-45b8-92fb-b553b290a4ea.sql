...
BEGIN
  ...
  /*
    Store-level latest snapshot to avoid undercount when sync by chunks,
    while still taking exactly one snapshot per store.
  */
  ...
  positions AS (
    SELECT
      p.sku,
      COALESCE(sm.size_code, p.sku) AS size_code,
