...
DROP FUNCTION IF EXISTS public.fn_allocation_engine(uuid, uuid, text, uuid[]);
DROP FUNCTION IF EXISTS public.fn_allocation_engine(uuid, uuid, text);

-- Recreate with p_store_ids parameter
...
BEGIN
  ...
  -- ★ STORE FILTER: filter by p_store_ids if provided
  ...
  IF p_run_type IN ('V1','both') THEN
    WITH v1_raw AS (
      ...
      WHERE (COALESCE(p.oh,0)+COALESCE(p.it,0)) = 0
        AND COALESCE(soh.toh,0) < COALESCE(s.capacity,999999)
        AND (NOT v_scarcity_active OR s.tier != 'C' OR COALESCE(sfs.system_total,0) > COALESCE((v_scarcity_weights->>'min_system_stock')::integer, 50))
    ),
    v1_ranked AS (
      SELECT *,
  ...
  IF p_run_type IN ('V2','both') THEN
    WITH v2_raw AS (
      SELECT f.id AS fid,f.fc_name,f.fc_code,f.is_core_hero,f.criticality_class,
        ...
        COALESCE(p.oh,0) AS cs,d.vel,d.co,d.so,COALESCE(soh.toh,0) AS stoh,
        CASE WHEN d.vel>0 THEN COALESCE(p.oh,0)/d.vel ELSE 999 END AS wc,
      ...
      WHERE (d.vel > 0 OR d.co > 0)
        AND (CASE WHEN d.vel>0 THEN COALESCE(p.oh,0)/d.vel ELSE 999 END) < v_min_cover_weeks * 7
        AND COALESCE(soh.toh,0) < COALESCE(s.capacity,999999)
    ...
    v2_ranked AS (
      SELECT *,
        GREATEST(1, CEIL(vel * v_min_cover_weeks * 7 - cs)::integer + co) AS need,
        ROW_NUMBER() OVER (PARTITION BY fid ORDER BY co DESC, vel DESC, tord) AS rn,
        SUM(GREATEST(1, CEIL(vel * v_min_cover_weeks * 7 - cs)::integer + co))
    ...
    SELECT p_tenant_id,p_run_id,fid,fc_name||' ('||fc_code||')',sid,store_name||' ('||store_code||')',
      ...
      CASE WHEN co>0 THEN 'V2-Đơn KH: '||co||' pcs, vel='||ROUND(vel,2)||'/ngày (×'||ROUND(dyn_weight,2)||')'
      ...
      jsonb_build_object('source_on_hand',cwt,'dest_on_hand',cs,'sold_7d',ROUND(vel*7,1),'customer_orders',co,'store_orders',so,'dyn_weight',ROUND(dyn_weight,2),'criticality',criticality_class),
      'V2'
    FROM v2_ok;
