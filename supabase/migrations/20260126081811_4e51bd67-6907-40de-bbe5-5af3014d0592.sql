...
-- ============================================================================
-- FIX V4: populate_finance_monthly_summary
-- Fix EXTRACT(DAY FROM integer) error - need to use proper date arithmetic
-- ============================================================================
    ...
    FROM (
      SELECT 
        ...
        CASE WHEN v_days_in_month > 0 
          THEN SUM(net_revenue) / v_days_in_month 
          ELSE 0 
        END as daily_revenue,
        CASE WHEN v_days_in_month > 0 
          THEN SUM(cogs) / v_days_in_month 
          ELSE 0 
        END as daily_cogs,
