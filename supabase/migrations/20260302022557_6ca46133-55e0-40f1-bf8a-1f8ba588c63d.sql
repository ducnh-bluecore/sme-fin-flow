...
-- Unified batch size split function: processes ALL null size_breakdown records
-- for both allocation and rebalance tables in a single DB call.
-- No more N+1 RPC calls from edge functions.

CREATE OR REPLACE FUNCTION public.fn_batch_size_split(
  p_tenant_id uuid,
  p_run_id uuid,
  p_table_name text,           -- 'alloc' or 'rebalance'
