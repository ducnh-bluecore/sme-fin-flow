...
-- Fix AR: only count recent in-transit orders (last 60 days), exclude completed-like statuses
...
BEGIN
  ...
  -- Exclude completed-like statuses explicitly (case-insensitive)
  ...
  WHERE tenant_id = p_tenant_id
    ...
    AND LOWER(status) NOT IN (
      ...
      '1', '5', '130', '140', '122', '121'  -- KiotViet completed/cancelled codes
    )
    AND LOWER(status) IN (
  ...
  -- 13. LTV
  SELECT COALESCE(AVG(cust_revenue) * AVG(cust_order_count), 0)
  INTO v_ltv
  FROM (
    SELECT customer_id, SUM(net_revenue) as cust_revenue, COUNT(*) as cust_order_count
    FROM cdp_orders
    WHERE tenant_id = p_tenant_id
      AND customer_id IS NOT NULL
      AND status NOT IN ('cancelled','refunded')
  ...
  INSERT INTO central_metrics_snapshots (
    ...
    computed_by, computation_duration_ms, created_at
  ) VALUES (
    ...
    'compute_central_metrics_snapshot', v_duration_ms, NOW()
  );
