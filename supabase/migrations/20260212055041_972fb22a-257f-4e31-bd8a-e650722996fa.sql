...
-- Update compute_central_metrics_snapshot: AR from cdp_orders (retail logic)
-- In retail/e-commerce: orders = invoices, AR = in-transit orders pending settlement
...
BEGIN
  ...
  -- =========================================================
  -- 3. AR from cdp_orders (RETAIL LOGIC)
  -- In retail/e-commerce: đơn hàng = hóa đơn
  -- AR = đơn đang vận chuyển / chờ sàn thanh toán (tiền chưa về)
  -- =========================================================
  SELECT 
    COALESCE(SUM(net_revenue), 0)
  INTO v_total_ar
  ...
  WHERE tenant_id = p_tenant_id
    AND LOWER(status) IN (
      '2', '3',                          -- Numeric statuses (KiotViet)
      'shipped', 'confirmed', 'packed',  -- Common e-commerce statuses
      'ready_to_ship', 'processed',      -- Marketplace statuses
      'to_confirm_receive',              -- TikTok/Shopee pending confirmation
      'delivering', 'in_transit'         -- Delivery statuses
    )
    AND status NOT IN ('cancelled', 'refunded', 'delivered', 'completed');

  -- AR aging buckets based on order age
  SELECT 
  ...
  -- Overdue AR = orders older than 14 days still in-transit (should have settled by now)
  ...
  -- =========================================================
  -- 5. Inventory from products (unchanged)
  -- =========================================================
  ...
  -- Platform hold = Retail AR (tiền sàn giữ)
  ...
  -- =========================================================
  -- 8. MARKETING from promotion_campaigns
  -- =========================================================
  SELECT 
