-- =====================================================
-- v_retail_concentration_risk: 5 loại rủi ro tập trung cho Bán lẻ E-commerce
-- =====================================================
...
sku_stats AS (
  SELECT 
    o.tenant_id,
    oi.product_id,
    COALESCE(p.name, 'SKU-' || LEFT(oi.product_id, 8)) as product_name,
    COALESCE(oi.category, 'Chưa phân loại') as category,
...
-- Grant access
GRANT SELECT ON v_retail_concentration_risk TO authenticated, anon;