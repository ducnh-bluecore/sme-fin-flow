...
DECLARE
  ...
  v_median_base numeric;  -- NEW: store pure median for mean-reversion check
  v_weighted_base numeric; -- NEW: store pure weighted for comparison
  i int;
  cohort_rec record;
  seasonal_rec record;
  yoy_rec record;
BEGIN
  ...
  -- === FIX 1: YoY GROWTH FACTORS — smoothed over 6-month rolling window ===
  v_yoy_growth_factors := '{}'::jsonb;
  FOR yoy_rec IN
    WITH monthly_rev AS (
      SELECT 
    ...
    -- Rolling 6-month avg for each year to smooth out spikes
    rolling_avg AS (
      SELECT
        m, month_num, yr, rev,
    ...
    yoy_pairs AS (
      SELECT 
        ...
        a.rev as this_rev,
        b.rev as last_rev,
        a.rolling6 as this_rolling,
        b.rolling6 as last_rolling,
        -- Blend: 50% direct YoY + 50% rolling YoY for smoothing
    ...
    SELECT 
      month_num,
      AVG(blended_yoy) as avg_yoy
    FROM yoy_pairs
    WHERE blended_yoy IS NOT NULL
    GROUP BY month_num
  LOOP
    -- Cap YoY between 0.7x and 2.0x (tighter cap)
  ...
  -- === FIX 2: DYNAMIC SEASONAL CAP — lower overall cap ===
  v_seasonal_multipliers := '{}'::jsonb;
  FOR seasonal_rec IN
    WITH monthly_rev AS (
      SELECT 
      ...
      CASE 
        WHEN sample_count >= 2 THEN 
          GREATEST(
            0.7, 
            LEAST(
              -- FIX: Lower dynamic cap: 1.2 + cv*0.5 (was 1.4 + cv)
              1.2 + COALESCE(cv, 0.3) * 0.5,
              seasonal_idx
            )
          )
  ...
  -- === FIX 3: TRAILING MEDIAN with mean-reversion damper ===
  ...
  -- Store pure median for mean-reversion check
  v_median_base := v_returning_base + v_anon_base;
  ...
  -- Blend: 60% median + 40% weighted avg
  DECLARE
  ...
  BEGIN
    ...
    v_weighted_base := v_weighted_ret + v_weighted_anon;
  ...
  FOR i IN 0..(p_horizon_months - 1) LOOP
    ...
    DECLARE
      ...
      v_mean_reversion_factor numeric;
    BEGIN
      -- Combined = seasonal * sqrt(yoy)
      v_combined_mult := v_seasonal_mult * POWER(v_yoy_factor, 0.5);
      
      -- === FIX 3: MEAN-REVERSION DAMPER ===
      -- If blended base is already >15% above median, dampen the combined multiplier
      -- This prevents double-amplification when recent months had spikes
      IF v_median_base > 0 AND v_weighted_base > v_median_base * 1.15 THEN
        -- Calculate how inflated the base is vs median
        v_mean_reversion_factor := v_median_base / v_weighted_base;
        -- Apply partial damping: sqrt to soften
        v_combined_mult := v_combined_mult * POWER(v_mean_reversion_factor, 0.5);
      END IF;
      
      -- FIX: Tighter final cap: 0.6x to 1.5x (was 2.0x)
