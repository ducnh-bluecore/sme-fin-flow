...
-- Fix: core_size_missing is boolean, shallow_depth_count is integer
...
BEGIN
  ...
  SELECT v_tid, d.fc_id::text, d.store_id, p_as_of_date,
    ...
    -- core_size_missing: boolean - true if any core size is missing
    (COUNT(DISTINCT d.size_code) FILTER (WHERE d.is_core AND d.on_hand<=v_min_stock) > 0),
    -- shallow_depth_count: integer
  ...
  RETURN jsonb_build_object('success',true,'as_of_date',p_as_of_date,'lost_revenue_rows',v_lr,'cash_lock_rows',v_cl,
