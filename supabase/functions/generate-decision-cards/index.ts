...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const allResults: { tenant_id: string; cards_created: number; job_id?: string; skipped?: boolean }[] = [];

    for (const tenant of tenants || []) {
      const tenantId = tenant.id;
      
      // ========== IDEMPOTENCY: Acquire job lock per tenant ==========
      const jobContext = await acquireJobLock(supabase, 'generate-decision-cards', tenantId, {
        grainDate: new Date().toISOString().split('T')[0],
      });

      if (!jobContext) {
        console.log(`[generate-decision-cards] Failed to acquire lock for tenant: ${tenantId}`);
        allResults.push({ tenant_id: tenantId, cards_created: 0, skipped: true });
        continue;
      }

      if (jobContext.alreadyRunning) {
        console.log(`[generate-decision-cards] Job already running for tenant: ${tenantId}`);
        allResults.push({ tenant_id: tenantId, cards_created: 0, skipped: true, job_id: jobContext.jobId });
        continue;
      }
      ...
      try {
        ...
        console.log(`[generate-decision-cards] Processing tenant: ${tenantId} (job: ${jobId})`);
        ...
        await completeJob(supabase, jobId, { cards_created: cardsCreated });

        allResults.push({ tenant_id: tenantId, cards_created: cardsCreated, job_id: jobId });
        console.log(`[generate-decision-cards] Tenant ${tenantId}: Created ${cardsCreated} cards (job: ${jobId})`);

      } catch (tenantError) {
        // ========== IDEMPOTENCY: Mark job as failed ==========
        const errorMessage = tenantError instanceof Error ? tenantError.message : 'Unknown error';
        await failJob(supabase, jobId, errorMessage);
        console.error(`[generate-decision-cards] Tenant ${tenantId} failed:`, errorMessage);
        allResults.push({ tenant_id: tenantId, cards_created: 0, job_id: jobId });
      }
    }
